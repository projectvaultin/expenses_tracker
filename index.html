...
<title>Expenses Tracker — by ProjectVault</title>
<link href="https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Sans:wght@300;400;500&display=swap" rel="stylesheet">
<style>
:root{--bg:#0f1117;--surface:#1a1d27;--surface2:#22263a;--border:rgba(255,255,255,0.08);--accent:#6c63ff;--red:#ff6584;--green:#00d084;--amber:#ffb300;--text:#f0f0f5;--muted:#7a7d99;--placeholder:rgba(122,125,153,.45);--card-shadow:none;--chip-shadow:none;--card-r:20px;--topbar-bg:rgba(15,17,23,.92);--tabs-bg:rgba(26,29,39,.97);--scheme:dark;}
html[data-theme="light"]{--bg:#eef0f6;--surface:#ffffff;--surface2:#f1f2f9;--border:rgba(20,21,29,0.16);--accent:#6c63ff;--red:#e0355a;--green:#00a86b;--amber:#c98200;--text:#14151d;--muted:#52546b;--placeholder:rgba(82,84,107,.55);--card-shadow:0 1px 3px rgba(20,21,29,.07),0 1px 2px rgba(20,21,29,.05);--chip-shadow:0 1px 2px rgba(20,21,29,.05);--card-r:20px;--topbar-bg:rgba(255,255,255,.92);--tabs-bg:rgba(255,255,255,.97);--scheme:light;}
html[data-theme="contrast"]{--bg:#000000;--surface:#0a0a0a;--surface2:#1a1a1a;--border:rgba(255,255,255,0.4);--accent:#ffdd00;--red:#ff5555;--green:#00ff88;--amber:#ffdd00;--text:#ffffff;--muted:#cccccc;--placeholder:rgba(204,204,204,.6);--card-shadow:none;--chip-shadow:none;--card-r:20px;--topbar-bg:rgba(0,0,0,.97);--tabs-bg:rgba(0,0,0,.98);--scheme:dark;}
html[data-theme="contrast"] .card,html[data-theme="contrast"] .hero{border-width:2px;}
html{transition:background-color .25s ease;}
html[data-textsize="large"] body{zoom:1.15;}
*{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;}
body{font-family:'DM Sans',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;overflow-x:hidden;font-weight:400;-webkit-font-smoothing:antialiased;transition:background-color .25s ease,color .25s ease;}
...
h1,h2,h3,.tab-label,.synebody,.card-title,.form-label,.fl-label{font-family:'DM Sans',sans-serif;}
/* THEME TOGGLE */
.theme-toggle{width:36px;height:36px;border-radius:50%;background:var(--surface2);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:16px;cursor:pointer;transition:all .15s;flex-shrink:0;}
.theme-toggle:active{transform:scale(.9);}
#rollover-switch::before{content:'';position:absolute;height:18px;width:18px;left:2px;top:2px;background:var(--muted);border-radius:50%;transition:.2s;}
#rollover-toggle:checked+#rollover-switch{background:var(--accent);border-color:var(--accent);}
#rollover-toggle:checked+#rollover-switch::before{transform:translateX(18px);background:#fff;}
#require2fa-switch::before{content:'';position:absolute;height:18px;width:18px;left:2px;top:2px;background:var(--muted);border-radius:50%;transition:.2s;}
#require2fa-toggle:checked+#require2fa-switch{background:var(--accent);border-color:var(--accent);}
#require2fa-toggle:checked+#require2fa-switch::before{transform:translateX(18px);background:#fff;}
  .pw-toggle{position:absolute;right:8px;top:50%;transform:translateY(-50%);cursor:pointer;display:flex;user-select:none;}
.pw-toggle svg{display:block;}
.pw-toggle .pw-hands{transform-origin:50px 50px;transition:transform .35s cubic-bezier(.34,1.56,.64,1),opacity .3s;}
.pw-toggle .pw-eyes{opacity:0;transform:scale(.3);transform-origin:50px 46px;transition:transform .3s cubic-bezier(.34,1.56,.64,1) .1s,opacity .2s .1s;}
.pw-toggle.pw-shown .pw-hands{transform:translateY(26px) rotate(-8deg);opacity:.15;}
.pw-toggle.pw-shown .pw-eyes{opacity:1;transform:scale(1);}
...
.side-drawer{position:fixed;top:0;right:0;bottom:0;width:78%;max-width:300px;background:var(--surface);border-left:1px solid var(--border);z-index:501;transform:translateX(100%);transition:transform .3s cubic-bezier(.4,0,.2,1);display:flex;flex-direction:column;padding:22px 16px 16px;overflow-y:auto;}
.side-drawer.open{transform:translateX(0);}
.drawer-profile{display:flex;flex-direction:column;align-items:center;text-align:center;padding-bottom:16px;border-bottom:1px solid var(--border);margin-bottom:10px;}
.drawer-av{width:60px;height:60px;border-radius:50%;background:var(--accent);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;color:#fff;overflow:hidden;margin-bottom:8px;border:2px solid rgba(108,99,255,.4);}
.drawer-item{display:flex;align-items:center;gap:12px;padding:13px 10px;border-radius:12px;cursor:pointer;font-size:14px;font-weight:600;color:var(--text);transition:background .15s;}
.drawer-item:active{background:var(--surface2);}
.drawer-item .di-icon{font-size:18px;width:22px;text-align:center;flex-shrink:0;}
.hamburger-btn{width:36px;height:36px;border-radius:12px;background:var(--surface2);border:1px solid var(--border);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;cursor:pointer;flex-shrink:0;box-shadow:var(--chip-shadow);}
.hamburger-btn span{width:16px;height:2px;background:var(--muted);border-radius:2px;}
@media (max-width:400px){
  .tb-name{font-size:12.5px;}
  .tb-av{width:30px;height:30px;font-size:12px;}
  .pill-btn{padding:5px 9px;font-size:10.5px;}
  .theme-toggle{width:30px;height:30px;font-size:14px;}
  .tb-right{gap:5px;}
  .hamburger-btn{width:30px;height:30px;border-radius:10px;}
  .hamburger-btn span{width:13px;}
}
/* AUTH */
#auth-screen{min-height:100vh;min-height:100dvh;display:none;flex-direction:column;align-items:center;justify-content:center;padding:24px 24px 32px;position:relative;}
.auth-logo-wrap{display:flex;flex-direction:column;align-items:center;margin-bottom:6px;}
.auth-logo-icon{width:56px;height:56px;border-radius:16px;margin-bottom:10px;box-shadow:0 6px 20px rgba(108,99,255,.35);}
.auth-logo{font-family:'Syne',sans-serif;font-size:clamp(21px,7vw,30px);font-weight:800;margin-bottom:0;letter-spacing:-.5px;white-space:nowrap;}
...
.auth-tab{flex:1;text-align:center;padding:9px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;color:var(--muted);border-radius:10px;cursor:pointer;transition:all .2s;}
...
.ai{width:100%;padding:12px 14px;background:var(--surface2);border:1px solid var(--border);border-radius:12px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:15px;outline:none;transition:border-color .2s;}
.ai:focus{border-color:var(--accent);}
.ai::placeholder{color:var(--placeholder);}
.auth-btn{width:100%;padding:14px;margin-top:6px;background:var(--accent);border:none;border-radius:14px;font-family:'DM Sans',sans-serif;font-size:15px;font-weight:700;color:#fff;cursor:pointer;transition:opacity .2s,transform .15s;}
...
/* Biometric btn */
.bio-btn{width:100%;padding:13px;margin-top:8px;background:var(--surface2);border:1px solid var(--border);border-radius:14px;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:700;color:var(--text);cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;transition:all .2s;}
.bio-btn:active{background:var(--accent);color:#fff;}
/* APP */
#app-screen{display:none;}
.topbar{position:sticky;top:0;z-index:100;background:var(--topbar-bg);backdrop-filter:blur(16px);padding:12px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;}
.tb-left{display:flex;align-items:center;gap:10px;}
.tb-av{width:36px;height:36px;border-radius:50%;background:var(--accent);display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-size:14px;font-weight:700;color:#fff;overflow:hidden;flex-shrink:0;border:2px solid rgba(108,99,255,.4);}
.tb-av img{width:100%;height:100%;object-fit:cover;}
.tb-name{font-size:15px;font-weight:700;white-space:nowrap;}
.tb-name span{color:var(--accent);}
.tb-sub{font-size:11px;color:var(--muted);margin-top:1px;}
.tb-right{display:flex;gap:7px;}
.pill-btn{background:var(--surface2);border:1px solid var(--border);border-radius:100px;padding:6px 12px;font-size:12px;font-weight:600;color:var(--muted);cursor:pointer;transition:all .15s;position:relative;box-shadow:var(--chip-shadow);}
...
/* TABS */
.tabs{position:fixed;bottom:0;left:0;right:0;z-index:100;background:var(--tabs-bg);backdrop-filter:blur(12px);border-top:1px solid var(--border);display:flex;}
.tab{position:relative;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:10px 0 14px;cursor:pointer;transition:color .2s;color:var(--muted);}
.tab-badge{position:absolute;top:6px;right:calc(50% - 18px);width:9px;height:9px;border-radius:50%;background:#ff6584;box-shadow:0 0 0 2px var(--bg,#0f1117);}
...
/* CARDS */
.card{background:var(--surface);border:1px solid var(--border);border-radius:var(--card-r);padding:15px;margin-bottom:11px;box-shadow:var(--card-shadow);}
.card-title{font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--muted);margin-bottom:11px;}
/* HOME HERO */
.hero{background:var(--surface);border:1px solid var(--border);border-radius:var(--card-r);padding:22px 18px;margin-bottom:11px;text-align:center;box-shadow:var(--card-shadow);}
.hero-label{font-size:12px;color:var(--muted);font-weight:700;letter-spacing:1px;text-transform:uppercase;}
.hero-amt{font-family:'Syne',sans-serif;font-size:40px;font-weight:800;color:var(--text);letter-spacing:-2px;margin:6px 0 3px;}
...
.dp-amt{font-family:'Syne',sans-serif;font-size:22px;font-weight:800;letter-spacing:-1px;}
...
.sel-btn{background:var(--surface2);border:1px solid var(--border);border-radius:12px;padding:10px 5px;font-size:13px;color:var(--muted);cursor:pointer;text-align:center;transition:all .15s;box-shadow:var(--chip-shadow);}
...
.rs-pre{position:absolute;left:15px;top:50%;transform:translateY(-50%);font-family:'Syne',sans-serif;font-size:22px;font-weight:700;color:var(--muted);}
.amt-inp{width:100%;padding:15px 15px 15px 38px;background:var(--surface2);border:1px solid var(--border);border-radius:16px;color:var(--text);font-family:'Syne',sans-serif;font-size:26px;font-weight:700;outline:none;transition:border-color .2s;}
.amt-inp:focus{border-color:var(--accent);}
.amt-inp::placeholder{color:var(--placeholder);}
/* Date input fix */
.date-inp{width:100%;padding:12px 14px;background:var(--surface2);border:1px solid var(--border);border-radius:14px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:15px;outline:none;transition:border-color .2s;margin-bottom:13px;-webkit-appearance:none;appearance:none;color-scheme:var(--scheme);}
...
.pay-chip{flex:1;padding:11px 6px;background:var(--surface2);border:1px solid var(--border);border-radius:12px;font-size:13px;font-weight:600;color:var(--muted);cursor:pointer;text-align:center;transition:all .15s;box-shadow:var(--chip-shadow);}
...
.save-btn{width:100%;padding:15px;background:var(--accent);border:none;border-radius:16px;font-family:'DM Sans',sans-serif;font-size:16px;font-weight:700;color:#fff;cursor:pointer;transition:opacity .2s,transform .15s;}
...
/* Swipe-to-delete (History tab) */
.swipe-wrap{position:relative;overflow:hidden;}
.swipe-wrap:last-child .entry-row{border-bottom:none;}
.swipe-delete-bg{position:absolute;top:0;right:0;bottom:0;width:90px;background:var(--red);display:flex;align-items:center;justify-content:center;font-size:20px;border-radius:12px;}
...
.entry-amt{font-family:'Syne',sans-serif;font-size:15px;font-weight:700;white-space:nowrap;}
...
.type-btn{flex:1;padding:12px;background:var(--surface2);border:1px solid var(--border);border-radius:14px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;color:var(--muted);cursor:pointer;text-align:center;transition:all .15s;}
...
.mchip{flex-shrink:0;padding:7px 14px;background:var(--surface2);border:1px solid var(--border);border-radius:100px;font-size:12px;font-family:'Syne',sans-serif;font-weight:600;color:var(--muted);cursor:pointer;transition:all .15s;box-shadow:var(--chip-shadow);}
...
.ti{flex:1;padding:11px 13px;background:var(--surface2);border:1px solid var(--border);border-radius:12px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:14px;outline:none;transition:border-color .2s;}
.ti:focus{border-color:var(--accent);}
.add-btn{padding:11px 16px;background:var(--accent);border:none;border-radius:12px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;color:#fff;cursor:pointer;white-space:nowrap;}
.add-btn:active{opacity:.8;}
.exp-btn{width:100%;padding:13px;background:transparent;border:1px solid var(--border);border-radius:13px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:14px;font-weight:600;cursor:pointer;text-align:left;display:flex;align-items:center;gap:11px;margin-bottom:7px;transition:background .15s;}
...
.modal-title{font-family:'Syne',sans-serif;font-size:18px;font-weight:700;margin-bottom:15px;}
...
@keyframes celBounce{0%{transform:scale(.4) rotate(-8deg);opacity:0}55%{transform:scale(1.08) rotate(3deg);opacity:1}75%{transform:scale(.96) rotate(-2deg)}100%{transform:scale(1) rotate(0)}}
@keyframes confettiFall{0%{transform:translateY(-40px) translateX(var(--cx,0)) rotate(0deg);opacity:1}100%{transform:translateY(340px) translateX(var(--cx,0)) rotate(var(--cr,360deg));opacity:0}}
.confetti-piece{position:absolute;top:0;left:50%;font-size:20px;pointer-events:none;animation:confettiFall 1.6s cubic-bezier(.25,.46,.45,.94) forwards;}
...
.undo-toast{position:fixed;bottom:80px;left:50%;transform:translateX(-50%) translateY(20px);background:var(--surface2);color:var(--text);border:1px solid var(--border);box-shadow:var(--chip-shadow);font-size:13px;font-weight:600;padding:9px 9px 9px 16px;border-radius:100px;opacity:0;transition:all .3s;white-space:nowrap;pointer-events:none;z-index:999;display:flex;align-items:center;gap:10px;}
.undo-toast.show{opacity:1;transform:translateX(-50%) translateY(0);pointer-events:auto;}
.undo-toast button{background:var(--accent);color:#fff;border:none;border-radius:100px;padding:6px 14px;font-size:12px;font-weight:700;cursor:pointer;}
...
.dfilter-chip{padding:6px 12px;background:var(--surface2);border:1px solid var(--border);border-radius:100px;font-size:12px;font-weight:600;color:var(--muted);cursor:pointer;transition:all .15s;box-shadow:var(--chip-shadow);}
...
.repay-inp{flex:1;padding:8px 12px;background:var(--surface);border:1px solid var(--border);border-radius:10px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:14px;outline:none;}
...
<!-- EmailJS — used to actually send monthly statement + password-reset OTP emails from the browser. -->
<!-- Setup (one-time, ~5 min): 1) Create a free account at https://www.emailjs.com  -->
<!-- 2) Add an Email Service (e.g. connect your Gmail) → copy the Service ID  -->
<!-- 3) Create two Email Templates (see EMAILJS_SETUP.md) → copy each Template ID  -->
<!-- 4) Account → General → copy your Public Key  -->
<!-- 5) Paste all three values into the EMAILJS_CONFIG block near the top of the <script> below. -->
<script src="https://cdn.jsdelivr.net/npm/@emailjs/browser@4/dist/email.min.js"></script>
  <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
</head>
<body>
<!-- SPLASH INTRO SCREEN -->
<div id="splash-screen" style="position:fixed;inset:0;z-index:9999;background:radial-gradient(circle at 50% 30%,#181c2b 0%,#0f1117 65%);display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity .5s ease;overflow:hidden;">
  <div id="splash-orb" style="position:absolute;width:340px;height:340px;border-radius:50%;background:radial-gradient(circle,rgba(108,99,255,.22) 0%,rgba(108,99,255,0) 70%);top:50%;left:50%;transform:translate(-50%,-50%);animation:splashOrbPulse 3.2s ease-in-out infinite;pointer-events:none;"></div>
  <img src="icon-192.png" alt="Expenses Tracker" style="width:88px;height:88px;border-radius:22px;box-shadow:0 10px 40px rgba(108,99,255,.4);animation:splashPop .7s cubic-bezier(.34,1.56,.64,1);position:relative;"/>
  <div style="font-family:'Syne',sans-serif;font-size:26px;font-weight:800;color:#f0f0f5;margin-top:18px;opacity:0;animation:splashFadeIn .6s ease .3s forwards;position:relative;">Expenses<span style="color:#6c63ff;"> Tracker</span></div>
  <div style="display:flex;align-items:center;gap:6px;margin-top:8px;opacity:0;animation:splashFadeIn .6s ease .5s forwards;position:relative;">
    <img src="brand-logo.png" alt="ProjectVault" style="height:14px;width:auto"/>
    <span style="font-size:11px;color:#7a7d99;letter-spacing:.4px;">by ProjectVault</span>
  </div>
  <div id="splash-greeting" style="font-size:15px;font-weight:700;color:#f0f0f5;margin-top:26px;opacity:0;animation:splashFadeIn .6s ease .7s forwards;position:relative;text-align:center;"></div>
  <div id="splash-motivation" style="font-size:12px;color:#7a7d99;margin-top:6px;max-width:260px;text-align:center;line-height:1.6;opacity:0;animation:splashFadeIn .6s ease .85s forwards;position:relative;"></div>
  <div style="width:140px;height:3px;background:rgba(255,255,255,.1);border-radius:3px;margin-top:28px;overflow:hidden;position:relative;">
    <div id="splash-bar" style="width:0%;height:100%;background:linear-gradient(90deg,#6c63ff,#00d084);border-radius:3px;"></div>
  </div>
  <div id="splash-status" style="font-size:11px;color:#7a7d99;margin-top:10px;letter-spacing:.3px;opacity:0;animation:splashFadeIn .6s ease 1s forwards;position:relative;"></div>
  <div id="splash-version" style="font-size:10px;color:#54576e;margin-top:18px;letter-spacing:.5px;position:absolute;bottom:22px;"></div>
</div>
<style>
...
@keyframes splashOrbPulse{0%,100%{opacity:.55;transform:translate(-50%,-50%) scale(1)}50%{opacity:.9;transform:translate(-50%,-50%) scale(1.08)}}
@keyframes splashRevealAnim{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
.splash-reveal{animation:splashRevealAnim .6s cubic-bezier(.4,0,.2,1);}
</style>

<!-- ═══════════ PIN/BIO LOCK SCREEN ═══════════ -->
<div id="lock-screen" style="min-height:100vh;min-height:100dvh;display:none;flex-direction:column;align-items:center;justify-content:center;padding:24px 24px 32px;background:var(--bg);position:relative;">
  <div class="theme-toggle" id="theme-toggle-lock" onclick="toggleTheme()" style="position:absolute;top:18px;right:18px" role="button" aria-label="Toggle dark or light theme">🌙</div>
  <div class="auth-logo-wrap" style="margin-bottom:6px"><img src="icon-192.png" class="auth-logo-icon" alt="Expenses Tracker"/><div class="auth-logo">Expenses<span> Tracker</span></div><div style="display:flex;align-items:center;gap:5px;margin-top:6px;opacity:.85;"><img src="brand-logo.png" alt="ProjectVault" style="height:16px;width:auto;"/><span style="font-size:11px;color:var(--muted);letter-spacing:.4px;">by <b style="color:var(--text)">ProjectVault</b></span></div></div>
  ...
  <div style="text-align:center;margin-top:18px;font-size:12px;color:var(--muted);letter-spacing:.3px;display:flex;align-items:center;justify-content:center;gap:6px;">Powered by <img src="brand-logo.png" alt="ProjectVault" style="height:20px;width:auto;vertical-align:middle;"/><span style="color:var(--accent);font-weight:700;">ProjectVault</span></div>
</div>

<!-- ═══════════ AUTH ═══════════ -->
<div id="auth-screen">
  <div class="theme-toggle" id="theme-toggle-auth" onclick="toggleTheme()" style="position:absolute;top:18px;right:18px" role="button" aria-label="Toggle dark or light theme">🌙</div>
  <div class="auth-logo-wrap"><img src="icon-192.png" class="auth-logo-icon" alt="Expenses Tracker"/><div class="auth-logo">Expenses<span> Tracker</span></div><div style="display:flex;align-items:center;gap:5px;margin-top:6px;opacity:.85;"><img src="brand-logo.png" alt="ProjectVault" style="height:16px;width:auto;"/><span style="font-size:11px;color:var(--muted);letter-spacing:.4px;">by <b style="color:var(--text)">ProjectVault</b></span></div></div>
  ...
  <div class="auth-card">
    ...
    <div id="form-signup" style="display:none">
      ...
      <div class="fl"><div class="fl-label">Email (Gmail) — for statements &amp; OTP</div><input class="ai" id="se" type="email" placeholder="you@gmail.com" autocomplete="email"/></div>
      <div class="fl"><div class="fl-label">Mobile Number (for your records)</div><input class="ai" id="sm" type="tel" placeholder="10-digit mobile number" maxlength="10"/></div>
...
<div id="app-screen">
  <div class="topbar">
    <div class="tb-left">
      <div class="hamburger-btn" onclick="openDrawer()" aria-label="Menu"><span></span><span></span><span></span></div>
      <div class="tb-av" id="tb-av"></div>
      <div>
        <div class="tb-name">Expenses<span> Tracker</span></div>
  ...
  <div class="side-drawer" id="side-drawer">
    <div class="drawer-profile">
      ...
      <div style="font-size:15px;font-weight:700;color:var(--text)" id="drawer-name"></div>
      <div style="font-size:12px;color:var(--muted)" id="drawer-username"></div>
      <div style="font-size:11px;color:var(--muted);opacity:0.75;margin-top:2px" id="drawer-userid">Account ID: checking…</div>
      <div style="font-size:11px;color:var(--muted);opacity:0.75;margin-top:2px" id="drawer-synccount">Cloud sync: checking…</div>
    </div>
    <div class="drawer-item" onclick="closeDrawer();showMonthPicker()"><span class="di-icon">📅</span> Filter by Month</div>
   <div class="drawer-item" onclick="closeDrawer();switchTab('manage',document.querySelectorAll('.tab')[4])">
    <span class="di-icon">⚙️</span> Manage Account
</div>
...
<div class="drawer-item" onclick="openSwitchAccountModal()">
    <span class="di-icon">🔀</span> Switch Account
</div>
    <div class="drawer-item" onclick="toggleTheme()"><span class="di-icon" id="drawer-theme-icon">🌙</span> <span id="drawer-theme-label">Switch to Light Theme</span></div>
    <div class="drawer-item" onclick="toggleTextSize()"><span class="di-icon">🔠</span> <span id="drawer-textsize-label">Switch to Large Text</span></div>
    <div class="drawer-item" onclick="toggleContrastMode()"><span class="di-icon">🔲</span> <span id="drawer-contrast-label">Turn On High Contrast</span></div>
    <div class="drawer-item" onclick="closeDrawer();showChangePwModal()"><span class="di-icon">🔑</span> Change Password</div>
    <div class="drawer-item" style="color:var(--red);margin-top:auto" onclick="doLogout()"><span class="di-icon">🚪</span> Logout</div>
    <div style="text-align:center;margin-top:16px;font-size:10px;color:var(--muted);display:flex;align-items:center;justify-content:center;gap:5px;">by <img src="brand-logo.png" style="height:14px" alt="ProjectVault"/> ProjectVault</div>
  </div>
  ...
  <div id="page-home" class="page active">
    ...
    <div class="update-banner" id="announcement-banner" style="display:none;background:linear-gradient(135deg,rgba(255,101,132,.18),rgba(255,179,0,.12));border-color:rgba(255,101,132,.4);animation:none;" onclick="dismissAnnouncement()">
      <span style="font-size:18px">📣</span>
      <div style="flex:1">
        <div style="font-size:14px;font-weight:700;color:#ff6584">Announcement</div>
        <div style="font-size:12px;color:var(--muted);margin-top:2px;line-height:1.5" id="announcement-text"></div>
      </div>
      <span style="color:var(--muted);font-size:16px">✕</span>
    </div>
    <div class="card" id="notif-prompt-card" style="display:none;background:linear-gradient(135deg,rgba(108,99,255,.12),rgba(0,208,132,.08));">
      <div style="display:flex;align-items:center;gap:10px">
        <span style="font-size:22px">🔔</span>
        <div style="flex:1">
          <div style="font-size:13px;font-weight:700">Turn on reminders</div>
          <div style="font-size:11px;color:var(--muted);margin-top:2px">Get a real popup when a recurring bill is due</div>
        </div>
        <button class="pill-btn" style="background:var(--accent);color:#fff" onclick="enableNotifications()">Enable</button>
      </div>
    </div>
    <div class="hero">
    ...
    <div class="card" id="upcoming-week-card" style="display:none;">
      <div class="card-title" style="margin-bottom:8px">📅 Upcoming This Week</div>
      <div id="upcoming-week-list"></div>
    </div>
    <div class="card" id="streak-card" style="display:none;text-align:center;background:linear-gradient(135deg,rgba(255,179,0,.1),rgba(108,99,255,.08));">
      <div style="font-size:22px;line-height:1" id="streak-flame">🔥</div>
      <div style="font-family:'Syne',sans-serif;font-size:20px;font-weight:800;margin-top:4px" id="streak-count">0 day streak</div>
      <div style="font-size:11px;color:var(--muted);margin-top:2px">Log something every day to keep it going</div>
    </div>
    <div class="card" style="text-align:center">
      <div class="dp-label" style="justify-content:center">🔗 Combined Total (Income + Expenses)</div>
      <div class="dp-amt" id="home-combined-total" style="font-size:26px">₹0</div>
      <div class="dp-count" id="home-combined-count">0 entries</div>
    </div>
    <div class="card" id="home-insight-card" style="display:none;padding:12px 16px;">
      <div style="display:flex;align-items:center;gap:10px;">
        <span style="font-size:20px" id="home-insight-emoji">📊</span>
        <div style="flex:1">
          <div style="font-size:12px;color:var(--muted)">Top spending category</div>
          <div style="font-size:14px;font-weight:700" id="home-insight-text">—</div>
        </div>
      </div>
    </div>
    <div class="card" id="cat-budget-warn-card" style="display:none;padding:12px 16px;border-color:rgba(255,101,132,.35)">
      <div style="font-size:12px;color:var(--red);font-weight:700;margin-bottom:6px">⚠️ Over category budget</div>
      <div id="cat-budget-warn-list" style="font-size:12px;color:var(--muted);line-height:1.8"></div>
    </div>
    <div class="card" id="budget-card" style="display:none">
      <div class="card-title">🎯 Monthly Budget</div>
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px">
        <span style="font-size:13px;color:var(--muted)" id="budget-spent-label">₹0 spent</span>
        <span style="font-size:13px;color:var(--muted)" id="budget-limit-label">of ₹0</span>
      </div>
      <div class="bar-wrap" style="height:8px;border-radius:8px">
        <div class="bar-fill" id="budget-bar-fill" style="height:8px;border-radius:8px;width:0%;background:var(--accent)"></div>
      </div>
      <div style="font-size:12px;margin-top:8px" id="budget-status-msg"></div>
      <div style="font-size:12px;margin-top:6px;padding-top:8px;border-top:1px solid var(--border);color:var(--muted)" id="budget-projection"></div>
    </div>
    <div class="dp-grid">
      <div class="dp-card">
  ...
  <div id="page-add" class="page">
    ...
    <div class="card">
      ...
      <div class="form-label">Tags <span style="color:var(--muted);font-weight:400">(optional, comma-separated)</span></div>
      <input type="text" class="ti" id="expense-tags" placeholder="e.g. work, Goa trip, urgent" style="width:100%;margin-bottom:13px"/>
      <div class="form-label">Receipt Photo <span style="color:var(--muted);font-weight:400">(optional)</span></div>
      <div id="receipt-preview-wrap" style="margin-bottom:13px">
        <div id="receipt-add-btn" onclick="document.getElementById('receipt-file').click()" style="display:flex;align-items:center;gap:8px;background:var(--surface2);border:1px dashed var(--border);border-radius:14px;padding:12px;cursor:pointer;font-size:13px;color:var(--muted)">
          <span style="font-size:18px">📎</span> Attach a photo of the receipt
        </div>
        <input type="file" id="receipt-file" accept="image/*" style="display:none" onchange="handleReceiptPhoto(event)"/>
      </div>
      <button class="save-btn" id="save-exp-btn" onclick="saveExpense()" style="background:var(--red)">Save Expense</button>
    </div>
    <!-- MONTHLY REMINDERS -->
    <div class="card">
      <div class="card-title">🔁 Recurring Bills &amp; Income</div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">Add recurring payments (rent, EMI, subscriptions) <b style="color:var(--text)">or</b> recurring income (salary, freelance retainer). You'll get reminded on the day — tap <b style="color:var(--text)">Mark Paid / Received</b> each month and it's logged automatically to the right place.</div>
      <button class="pill-btn" style="margin-bottom:12px" onclick="sendTestReminderAlert()">🔔 Send Test Alert</button>
      <div id="reminders-list"></div>
      <div style="margin-top:12px">
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <div class="pay-chip sel" id="remtype-expense" onclick="selectRemType('expense')">💸 Expense</div>
          <div class="pay-chip" id="remtype-income" onclick="selectRemType('income')">💰 Income (e.g. Salary)</div>
        </div>
        <input type="text" class="ti" id="rem-name" placeholder="e.g. PG Rent, EMI, Netflix…" style="width:100%;margin-bottom:8px"/>
        <select class="ti" id="rem-freq" style="width:100%;margin-bottom:8px" onchange="onRemFreqChange()">
          <option value="monthly">Repeats Monthly</option>
          <option value="weekly">Repeats Weekly</option>
          <option value="quarterly">Repeats Quarterly (every 3 months)</option>
        ...
        <div style="display:flex;gap:8px;margin-bottom:8px">
          ...
          <select class="ti" id="rem-weekday" style="flex:1;display:none">
            <option value="0">Sunday</option><option value="1">Monday</option><option value="2">Tuesday</option>
            <option value="3">Wednesday</option><option value="4">Thursday</option><option value="5">Friday</option><option value="6">Saturday</option>
          </select>
          <input type="date" class="ti" id="rem-yearly-date" style="flex:1;display:none"/>
        </div>
  ...
  <div id="page-history" class="page">
    <div style="position:relative;margin-bottom:11px">
      <span style="position:absolute;left:13px;top:50%;transform:translateY(-50%);color:var(--muted);font-size:15px">🔍</span>
      <input type="text" class="ti" id="hist-search" placeholder="Search category, amount, payment mode, date…" style="width:100%;padding-left:36px" oninput="onHistSearch(this.value)"/>
    </div>
    <div class="mscroll" id="hist-months"></div>
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;cursor:pointer" onclick="toggleDateRangeReport()">
        <div class="card-title" style="margin-bottom:0">📆 Custom Date-Range Report</div>
        <span id="date-range-caret" style="color:var(--muted)">▾</span>
      </div>
      <div id="date-range-body" style="display:none;margin-top:12px">
        <div style="display:flex;gap:8px;margin-bottom:10px">
          <div style="flex:1"><div class="fl-label">From</div><input type="date" class="date-inp" id="range-from" style="margin-bottom:0"/></div>
          <div style="flex:1"><div class="fl-label">To</div><input type="date" class="date-inp" id="range-to" style="margin-bottom:0"/></div>
        </div>
        <button class="add-btn" style="width:100%;padding:10px" onclick="showDateRangeReport()">Show Report</button>
        <div id="date-range-result" style="margin-top:12px"></div>
      </div>
    </div>
  ...
  <div id="page-manage" class="page">
    ...
    <div class="card" style="padding:0;overflow:hidden;">
      <div id="cover-photo-box" onclick="document.getElementById('cover-photo-file').click()" style="height:100px;background:linear-gradient(135deg,rgba(108,99,255,.25),rgba(0,208,132,.15));background-size:cover;background-position:center;display:flex;align-items:center;justify-content:center;cursor:pointer;position:relative;">
        <span id="cover-photo-ph" style="font-size:12px;color:var(--muted);background:rgba(0,0,0,.4);padding:6px 12px;border-radius:100px;">📷 Tap to add a cover photo</span>
      </div>
      <input type="file" id="cover-photo-file" accept="image/*" style="display:none" onchange="handleCoverPhoto(event)"/>
      <div style="padding:16px">
      <div class="card-title">👤 Edit Profile</div>
      <div style="display:flex;gap:8px;margin-bottom:14px" id="profile-stats-row"></div>
      ...
      <div class="fl"><div class="fl-label">Email (Gmail) — for statements &amp; OTP</div><input class="ai" id="edit-email" type="email" placeholder="you@gmail.com"/></div>
    ...
    <div class="card">
      <div class="card-title">🎭 Your Money Personality</div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">A fun, light-hearted read on your spending habits based on your logged entries.</div>
      <button class="save-btn" style="background:var(--surface2);color:var(--text)" onclick="showMoneyPersonality()">✨ Reveal My Money Personality</button>
    </div>
    <!-- MONTHLY STATEMENT -->
    <div class="card">
      <div class="card-title">📧 Monthly Email Statement</div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">
        A summary of last month's income, expenses, and your average spend for the month &amp; year is emailed to your registered address automatically the next time you open the app after month-end.
        <span id="emailjs-status-note"></span>
      </div>
      <button class="save-btn" style="background:var(--surface2);color:var(--text)" onclick="sendTestStatementNow()">📤 Send Last Month's Statement Now</button>
    </div>
    <div class="card" id="install-card" style="display:none">
      <div class="card-title">📲 Install App</div>
      <div id="install-card-body">
        <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">Install this app on your device for quick access and offline use, just like a native app.</div>
        <button class="exp-btn" id="install-btn" onclick="doInstallPWA()">
          <span style="font-size:20px">⬇️</span>
          <div><div>Install to Home Screen</div><div style="font-size:11px;color:var(--muted);margin-top:2px">Adds an app icon, opens full-screen</div></div>
        </button>
      </div>
      <div id="install-card-installed" style="display:none;font-size:13px;color:var(--green);font-weight:600;align-items:center;gap:8px">
        ✅ Already installed on this device
      </div>
    </div>
    <div class="card">
      <div class="card-title">🎯 Monthly Budget Limit</div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">Set an overall spending limit for each month. You'll see a progress bar on Home and a warning if you go over.</div>
      <div style="position:relative;margin-bottom:10px">
        ...
        <input type="number" class="ti" id="budget-limit-input" placeholder="e.g. 20000" style="padding-left:30px;width:100%"/>
      </div>
      <button class="add-btn" style="width:100%;padding:12px" onclick="saveBudgetLimit()">💾 Save Budget</button>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-top:12px;padding-top:12px;border-top:1px solid var(--border)">
        <div><div style="font-size:13px;font-weight:600">🔄 Roll over unused budget</div><div style="font-size:11px;color:var(--muted);margin-top:2px">Money left over this month adds to next month's limit</div></div>
        <label style="position:relative;display:inline-block;width:42px;height:24px;flex-shrink:0">
          <input type="checkbox" id="rollover-toggle" onchange="setBudgetRollover(this.checked)" style="opacity:0;width:0;height:0">
          <span id="rollover-switch" style="position:absolute;cursor:pointer;inset:0;background:var(--surface2);border:1px solid var(--border);border-radius:24px;transition:.2s"></span>
        </label>
      </div>
    </div>
    <div class="card">
      <div class="card-title">🎯 Per-Category Budgets</div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">Optional finer limits — e.g. cap Food at ₹5,000/month even if your overall budget is higher. Leave blank for no limit.</div>
    ...
    <div class="card">
      ...
      <button class="exp-btn" id="bio-toggle-btn" onclick="toggleBiometricSetting()">
        <span style="font-size:20px" id="bio-toggle-icon">👆</span>
        <div><div id="bio-toggle-title">Enable Biometric Login</div><div style="font-size:11px;color:var(--muted);margin-top:2px" id="bio-toggle-sub">Use fingerprint / Face unlock instead of password</div></div>
      </button>
      <button class="exp-btn" onclick="confirmDeleteAccount()">
        <span style="font-size:20px">⚠️</span>
        <div><div style="color:var(--red)">Delete Account</div><div style="font-size:11px;color:var(--muted);margin-top:2px">Permanently erase your account and all data</div></div>
      </button>
      <div style="display:flex;align-items:center;justify-content:space-between;margin:12px 0;padding-top:12px;border-top:1px solid var(--border)">
        <div><div style="font-size:13px;font-weight:600">🔐 Two-Factor Login</div><div style="font-size:11px;color:var(--muted);margin-top:2px">Require an emailed code every time you log in</div></div>
        <label style="position:relative;display:inline-block;width:42px;height:24px;flex-shrink:0">
          <input type="checkbox" id="require2fa-toggle" onchange="toggleRequire2FA(this.checked)" style="opacity:0;width:0;height:0">
          <span id="require2fa-switch" style="position:absolute;cursor:pointer;inset:0;background:var(--surface2);border:1px solid var(--border);border-radius:24px;transition:.2s"></span>
        </label>
      </div>
      <div style="font-size:11px;color:var(--muted);margin:6px 0 8px;">🔒 Auto-locks after 5 minutes of inactivity</div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:6px;font-weight:700">🕒 Recent Logins (this device)</div>
      <div id="login-activity-list" style="font-size:12px;color:var(--muted);line-height:1.9"></div>
    </div>
    <!-- Admin Dashboard / Statistics (Admin Only) -->
    <div class="card" id="admin-stats-section" style="display:none">
      <div class="card-title">📊 Application Statistics <span style="font-size:10px;color:var(--accent);font-weight:700;padding:2px 6px;background:rgba(108,99,255,.15);border-radius:6px;margin-left:4px">ADMIN</span></div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:12px;line-height:1.6" id="admin-stats-source">Loading…</div>
      <div id="admin-stats-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:10px"></div>
      <button class="pill-btn" style="margin-top:12px" onclick="renderAdminStats()">🔄 Refresh</button>
    </div>
   <div class="card" id="support-user-card">
  <div class="card-title">💬 Contact Admin</div>
  <div class="card-title">🎫 Customer Support Center</div>

  <div style="font-size:12px;color:var(--muted);margin-bottom:14px;line-height:1.6">
    Need help? Report bugs, request new features, or contact ProjectVault support.
    Every message becomes a support ticket that you can track.
  </div>

  <div class="fl">
    <div class="fl-label">Category</div>
    <select class="ti" id="support-category">
      <option value="Bug Report">🐞 Bug Report</option>
      <option value="Feature Request">✨ Feature Request</option>
      <option value="Account Issue">👤 Account Issue</option>
      <option value="Payment Issue">💳 Payment Issue</option>
      <option value="Sync Problem">☁️ Sync Problem</option>
  ...
  <div class="fl">
    <div class="fl-label">Priority</div>
    <select class="ti" id="support-priority">
  ...
  <div class="fl">
    <div class="fl-label">Subject</div>
    <input
      type="text"
      class="ai"
      id="support-subject"
  ...
  <div class="fl">
    <div class="fl-label">Describe your issue</div>
    <textarea
      class="ti"
      id="support-message"
  ...
  <div
    ...
    <div class="card" id="admin-members-section" style="display:none">
      <div class="card-title">👥 Manage Users <span style="font-size:10px;color:var(--accent);font-weight:700;padding:2px 6px;background:rgba(108,99,255,.15);border-radius:6px;margin-left:4px">ADMIN</span></div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:12px;line-height:1.6" id="admin-members-note">Loading account source…</div>
      <div id="admin-members-summary" style="font-size:13px;font-weight:700;color:var(--accent);margin-bottom:10px"></div>
      <div id="admin-members-list"></div>
    </div>
    <!-- Customer messages (Admin Only) -->
    <div class="card" id="admin-messages-section" style="display:none">
      <div class="card-title">📨 Customer Messages <span style="font-size:10px;color:var(--accent);font-weight:700;padding:2px 6px;background:rgba(108,99,255,.15);border-radius:6px;margin-left:4px">ADMIN</span></div>
      <div id="admin-messages-list"></div>
    </div>
    <!-- Error Logs (Admin Only) -->
    <div class="card" id="admin-errorlogs-section" style="display:none">
      <div class="card-title">🐞 Error Logs <span style="font-size:10px;color:var(--accent);font-weight:700;padding:2px 6px;background:rgba(108,99,255,.15);border-radius:6px;margin-left:4px">ADMIN</span></div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:12px;line-height:1.6">Runtime errors captured on <b style="color:var(--text)">this device</b> — useful for debugging what's actually going wrong, since this is a client-only app with no central server log.</div>
      <div id="admin-errorlogs-list"></div>
      <button class="pill-btn" style="margin-top:10px" onclick="clearErrorLogs()">🗑 Clear Logs</button>
    </div>
    <!-- Announcements (Admin Only) — broadcast, independent of version release notes -->
    <div class="card" id="admin-announcement-section" style="display:none">
      <div class="card-title">📣 Send Announcement <span style="font-size:10px;color:var(--accent);font-weight:700;padding:2px 6px;background:rgba(108,99,255,.15);border-radius:6px;margin-left:4px">ADMIN</span></div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">A quick broadcast to all users — for things like scheduled maintenance or a heads-up, separate from version release notes below.</div>
      <textarea class="ti" id="announcement-text-inp" placeholder="e.g. Scheduled maintenance tonight 11pm–12am IST" style="width:100%;min-height:70px;resize:none;line-height:1.6;margin-bottom:8px"></textarea>
      <button class="add-btn" style="width:100%;padding:12px" onclick="pushAnnouncement()">📣 Publish Announcement</button>
      <div id="admin-announcement-sync-note" style="font-size:11px;color:var(--muted);margin-top:8px;line-height:1.5"></div>
    </div>
    <!-- App Update Management (Admin Only) -->
    <div class="card" id="admin-update-section" style="display:none">
      <div class="card-title">📢 Push App Update Notice <span style="font-size:10px;color:var(--accent);font-weight:700;padding:2px 6px;background:rgba(108,99,255,.15);border-radius:6px;margin-left:4px">ADMIN</span></div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">Notify all users of a new version. They'll see a banner next time they open the app.</div>
      <input type="text" class="ti" id="update-title-inp" placeholder="Title e.g. Faster sync & new charts" style="width:100%;margin-bottom:8px"/>
      <input type="text" class="ti" id="update-version-inp" placeholder="Version e.g. 2.1" style="width:100%;margin-bottom:8px"/>
      <textarea class="ti" id="update-notes-inp" placeholder="What's new in this update..." style="width:100%;min-height:80px;resize:none;line-height:1.6"></textarea>
      <button class="add-btn" style="width:100%;margin-top:8px;padding:12px" onclick="pushUpdateNotice()">📢 Publish Update Notice</button>
    </div>
    <!-- Database Management (Admin Only) -->
    <div class="card" id="admin-database-section" style="display:none">
      <div class="card-title">🗄️ Manage Database <span style="font-size:10px;color:var(--accent);font-weight:700;padding:2px 6px;background:rgba(108,99,255,.15);border-radius:6px;margin-left:4px">ADMIN</span></div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;line-height:1.6">Export a full backup of every table the app can read. Large accounts may take a few seconds.</div>
      <button class="exp-btn" onclick="exportFullDatabaseBackup()">
        <span style="font-size:20px">☁️</span>
        <div><div>Export Full Database Backup</div><div style="font-size:11px;color:var(--muted);margin-top:2px">All users, transactions, debts &amp; reminders → one .json file</div></div>
      </button>
      <div style="font-size:11px;color:var(--muted);margin-top:4px;line-height:1.5">If your Supabase Row-Level Security policies restrict reads to each user's own rows, this export will only contain what the admin account itself can see — grant the admin account broader read access in Supabase if you need a truly complete backup.</div>
    </div>
  </div>
  ...
  <nav class="tabs" role="tablist" aria-label="Main navigation">
    <div class="tab active" onclick="switchTab('home',this)" role="tab" aria-selected="true" aria-label="Home">
    ...
    <div class="tab" onclick="switchTab('add',this)" role="tab" aria-selected="false" aria-label="Add expense or income">
    ...
    <div class="tab" onclick="switchTab('history',this)" role="tab" aria-selected="false" aria-label="History">
      <svg viewBox="0 0 24 24" aria-hidden="true"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
      <span class="tab-label">History</span>
    </div>
    <div class="tab" onclick="switchTab('debt',this)" role="tab" aria-selected="false" aria-label="Debts">
    ...
    <div class="tab" onclick="switchTab('manage',this)" role="tab" aria-selected="false" aria-label="Manage account">
      <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M4.93 4.93a10 10 0 0 0 0 14.14"/></svg>
      <span id="manage-reply-badge" class="tab-badge" style="display:none"></span>
...
<div class="modal-overlay" id="edit-entry-modal">
  <div class="modal">
    ...
    <div class="form-label">Month</div>
    <div class="sgrid m3" id="edit-month-grid"></div>
    <div class="form-label">Category</div>
    <div class="sgrid" id="edit-cat-grid"></div>
    <div class="form-label">Amount</div>
    <div class="amt-wrap">
      <span class="rs-pre" id="edit-amt-sign">₹</span>
      <input type="number" inputmode="decimal" class="amt-inp" id="edit-amount-input" placeholder="0"/>
    </div>
    <div class="form-label">Date</div>
    <input type="date" id="edit-date-input" class="date-inp" style="padding-left:14px;font-size:15px;"/>
    <button class="save-btn" onclick="saveEditEntry()">💾 Save Changes</button>
    <button class="save-btn" style="background:var(--surface2);color:var(--muted);margin-top:8px" onclick="closeEditEntry()">Cancel</button>
  </div>
</div>
...
<div class="modal-overlay" id="update-modal">
  <div class="modal">
    ...
    <div id="update-modal-content" style="font-size:14px;color:var(--text);line-height:1.7;white-space:pre-line;margin-bottom:16px"></div>
    <button class="save-btn" onclick="dismissUpdate()">Got it! Mark as Read ✓</button>
  </div>
</div>
...
<div class="modal-overlay" id="switch-account-modal">
  <div class="modal">
    ...
    <div id="switch-account-list" style="max-height:280px;overflow-y:auto;margin-bottom:10px"></div>
    <button class="save-btn" style="background:var(--surface2);color:var(--muted)" onclick="document.getElementById('switch-account-modal').classList.remove('open')">Cancel</button>
  </div>
</div>

<!-- TWO-FACTOR LOGIN CHALLENGE -->
<div class="modal-overlay" id="login2fa-modal" style="align-items:center">
  <div style="background:var(--surface);border-radius:24px;padding:28px 24px;width:100%;max-width:320px;text-align:center;margin:20px">
    <div style="font-size:36px;margin-bottom:10px">🔒</div>
    <div style="font-size:17px;font-weight:800;margin-bottom:6px">Two-Factor Verification</div>
    <div style="font-size:12px;color:var(--muted);margin-bottom:16px;line-height:1.6">We emailed a 6-digit code to your registered email. Enter it below to finish logging in.</div>
    <input class="ai" id="login2fa-code" type="text" inputmode="numeric" maxlength="6" placeholder="000000" style="text-align:center;font-size:20px;letter-spacing:6px;margin-bottom:10px"/>
    <div class="auth-err" id="login2fa-err"></div>
    <button class="auth-btn" onclick="verifyLogin2FA()">Verify & Continue</button>
    <div style="margin-top:10px;font-size:12px;color:var(--muted);cursor:pointer" onclick="cancel2FA()">Cancel</div>
  </div>
</div>

<!-- ONBOARDING WALKTHROUGH -->
<div class="modal-overlay" id="onboarding-modal" style="align-items:center">
  <div style="background:var(--surface);border-radius:24px;padding:28px 24px;width:100%;max-width:340px;text-align:center;margin:20px">
    <div id="onboard-step-0" class="onboard-step">
      <div style="font-size:44px;margin-bottom:12px">👋</div>
      <div style="font-size:19px;font-weight:800;margin-bottom:8px">Welcome to Expenses Tracker!</div>
      <div style="font-size:13px;color:var(--muted);line-height:1.7">A quick 4-step tour of what you can do — takes 20 seconds.</div>
    </div>
    <div id="onboard-step-1" class="onboard-step" style="display:none">
      <div style="font-size:44px;margin-bottom:12px">💸</div>
      <div style="font-size:17px;font-weight:800;margin-bottom:8px">Track income & expenses</div>
      <div style="font-size:13px;color:var(--muted);line-height:1.7">Use the <b style="color:var(--text)">Add</b> tab to log spending by category, with tags, receipts, and payment method.</div>
    </div>
    <div id="onboard-step-2" class="onboard-step" style="display:none">
      <div style="font-size:44px;margin-bottom:12px">🎯</div>
      <div style="font-size:17px;font-weight:800;margin-bottom:8px">Set a budget</div>
      <div style="font-size:13px;color:var(--muted);line-height:1.7">Go to <b style="color:var(--text)">Manage</b> to set a monthly limit — overall or per category — and get warned before you overspend.</div>
    </div>
    <div id="onboard-step-3" class="onboard-step" style="display:none">
      <div style="font-size:44px;margin-bottom:12px">🔁</div>
      <div style="font-size:17px;font-weight:800;margin-bottom:8px">Never miss a bill</div>
      <div style="font-size:13px;color:var(--muted);line-height:1.7">Add recurring bills or salary — weekly, monthly, or yearly — and get reminded automatically.</div>
    </div>
    <div id="onboard-step-4" class="onboard-step" style="display:none">
      <div style="font-size:44px;margin-bottom:12px">☰</div>
      <div style="font-size:17px;font-weight:800;margin-bottom:8px">Everything else lives here</div>
      <div style="font-size:13px;color:var(--muted);line-height:1.7">Tap the menu icon (top-left) for your profile, theme, and account settings any time.</div>
    </div>
    <div style="display:flex;gap:6px;justify-content:center;margin:20px 0 16px" id="onboard-dots"></div>
    <button class="auth-btn" onclick="onboardNext()" id="onboard-next-btn">Next →</button>
    <div style="margin-top:10px;font-size:12px;color:var(--muted);cursor:pointer" onclick="closeOnboarding()">Skip</div>
  </div>
</div>

<!-- RECEIPT LIGHTBOX -->
<div class="modal-overlay" id="receipt-modal" style="align-items:center" onclick="if(event.target===this)this.classList.remove('open')">
  <div style="max-width:90vw;max-height:80vh;text-align:center">
    <img id="receipt-modal-img" src="" style="max-width:100%;max-height:70vh;border-radius:14px;border:1px solid var(--border)"/>
    <button class="save-btn" style="background:var(--surface2);color:var(--muted);margin-top:14px" onclick="document.getElementById('receipt-modal').classList.remove('open')">Close</button>
  </div>
</div>
...
<div class="modal-overlay" id="forgotpw-modal">
  <div class="modal">
    ...
    <div id="fp-step1">
      <div style="font-size:13px;color:var(--muted);margin-bottom:14px;line-height:1.6">Forgot your username or password? Enter your registered mobile number or email — we'll email you an OTP to verify.</div>
      <div class="fl"><div class="fl-label">Mobile Number or Email</div><input class="ai" id="fp-identifier" type="text" placeholder="10-digit mobile or you@gmail.com"/></div>
    ...
    <div id="fp-step2" style="display:none">
      <div style="font-size:13px;color:var(--muted);margin-bottom:14px;line-height:1.6">An OTP has been emailed to your registered address. Enter it below.</div>
      <div style="background:rgba(108,99,255,.1);border:1px solid rgba(108,99,255,.3);border-radius:12px;padding:12px;margin-bottom:14px;text-align:center">
        ...
        <div style="font-size:11px;color:var(--muted);margin-top:4px" id="fp-username-hint"></div>
      </div>
...
<!-- ACHIEVEMENT / MILESTONE CELEBRATION MODAL (debt cleared, budget goal met, savings milestone) -->
<div id="celebrate-modal" style="display:none;position:fixed;inset:0;z-index:400;background:rgba(0,0,0,.85);backdrop-filter:blur(6px);align-items:center;justify-content:center;padding:20px;overflow:hidden">
  <div id="celebrate-confetti-layer" style="position:absolute;inset:0;overflow:hidden;pointer-events:none"></div>
  <div id="celebrate-card" style="position:relative;background:linear-gradient(135deg,rgba(0,208,132,.2),rgba(108,99,255,.2));border:2px solid var(--green);border-radius:28px;padding:32px 24px;width:100%;max-width:340px;text-align:center;animation:celBounce .5s cubic-bezier(.34,1.56,.64,1)">
    <div style="font-size:56px;margin-bottom:12px" id="cel-emoji">🎉</div>
    <div style="font-size:22px;font-weight:800;color:var(--green);margin-bottom:8px" id="celebrate-title">DEBT CLEARED! 🎊</div>
    ...
    <button class="save-btn" id="celebrate-btn" style="background:var(--green);color:#052e1c;font-weight:800;font-size:16px" onclick="closeCelebration()">Woohoo! 🙌</button>
  </div>
</div>

<div class="toast" id="toast"></div>
<div class="undo-toast" id="undo-toast"><span id="undo-toast-msg"></span><button onclick="performUndo()">Undo</button></div>
...
const EMAILJS_CONFIG={
  ...
  otpTemplateId:'template_5mn6y4g',       // Template for password-reset OTP
...
let editIndex=null,editMonth=null,editCat=null;
...
if(users[CU]){
    ...
    console.log("Saved avatar length:", users[CU].avatar.length);

    saveUsers(users);

    const checkUsers = getUsers();

    console.log("Reloaded avatar length:", checkUsers[CU].avatar ? checkUsers[CU].avatar.length : null);
...
// ── PASSWORD SECURITY ──
// Passwords are now stored as a proper one-way SHA-256 hash (prefixed "sha256:") instead of the
// old btoa() encoding, which was just Base64 — trivially reversible by anyone with access to the
// browser's localStorage, not a real hash. verifyAndMigratePassword() checks new-style hashes
// normally, and if it finds an old-style account, verifies against the legacy encoding ONE time
// and transparently upgrades it to a real hash on successful login — no user action needed, no
// forced password reset, and old accounts keep working exactly as before from the user's side.
async function hashPassword(plain){
  if(!window.crypto||!window.crypto.subtle){
    console.warn('Web Crypto API unavailable in this environment — falling back to weaker encoding.');
    return btoa(plain);
  }
...
async function verifyAndMigratePassword(users,uname,plain){
  const acc=users[uname];
  if(!acc||!acc.password)return false;
  if(acc.password.startsWith('sha256:')){
    return acc.password===await hashPassword(plain);
  }
  // Legacy account (old btoa encoding) — verify against it, then silently upgrade to a real hash.
  if(acc.password===btoa(plain)){
    acc.password=await hashPassword(plain);
    saveUsers(users);
    return true;
  }
}
...
users[u].password = user.password_hash;
...
let _login2FAUser='',_login2FACode='';
...
async function ensureAdminAccount(){
  ...
  const ADMIN_USERNAME='admin'; function isAdmin(){ return CU === ADMIN_USERNAME; }
...
// Keep local copy (temporary until full migration)
users[u] = {
  name,
...
function confirmDeleteAccount(){
  ...
  const typed=prompt('Type DELETE to confirm permanently erasing the account "'+CU+'":');
  if(typed!=='DELETE'){showToast('Cancelled — account not deleted','#ffb300');return;}
  const deletedUser=CU;
  const users=getUsers();
  ...
  document.getElementById('auth-screen').style.display='flex';
  switchAuthTab('login');
  alert('Your account and all associated data have been permanently deleted from this device.');
}
...
async function doUnlock(){
  ...
  if(!await verifyAndMigratePassword(users,session,p)){
...
function openSwitchAccountModal(){
  closeDrawer();
  const users=getUsers();
  const others=Object.keys(users).filter(u=>u!==CU&&!(users[u].isAdmin&&u===ADMIN_USERNAME));
  ...
  if(!others.length){
    ...
    list.innerHTML=others.map(u=>{
      const p=users[u];
      const av=p.avatar?`<img src="${p.avatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`:(p.name||u).charAt(0).toUpperCase();
      return `<div class="manage-row" style="cursor:pointer" onclick="switchToAccount('${u}')">
        <div class="tb-av" style="margin-right:10px">${av}</div>
        <div style="flex:1"><div style="font-size:14px;font-weight:600">${p.name||u}</div><div style="font-size:11px;color:var(--muted)">@${u}</div></div>
        <span style="color:var(--muted);font-size:16px">›</span>
      </div>`;
    }).join('');
  }
...
function renderUpcomingWeek(){
  ...
  document.getElementById('upcoming-week-list').innerHTML=upcoming.map((r,i)=>{
    ...
    return `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;${last?'':'border-bottom:1px solid var(--border)'}">
      ...
      <div style="flex:1"><div style="font-size:13px;font-weight:600">${r.name}</div><div style="font-size:11px;color:var(--muted)">${label} · ${fmt(r.amount)}</div></div>
    </div>`;
  }).join('');
}
function renderHome(){
  ...
  if(Object.keys(catBudgets).length){
    ...
    if(overs.length){
      ...
      document.getElementById('cat-budget-warn-list').innerHTML=overs.map(([cat,lim])=>`${emoji(cat)} ${cat}: ${fmt(curMonthByCat[cat])} of ${fmt(lim)}`).join('<br>');
    } else {
    ...
    img.onload=()=>{
      ...
      document.getElementById('receipt-preview-wrap').innerHTML=`
        <div style="position:relative;display:inline-block">
          <img src="${pendingReceipt}" style="max-width:100px;max-height:100px;border-radius:10px;border:1px solid var(--border)"/>
...
function renderHistory(){
  ...
  let html='';
  if(!sorted.length)html=`<div class="empty"><div class="empty-icon">📭</div><p>No entries found${histSearch.trim()?' for "'+histSearch.trim()+'"':''}.</p></div>`;
  sorted.forEach(e=>{
    ...
    const debtTag=e.debtLinked?` · 🔁 ${e.debtName||'Debt'} repayment`:'';
    const tagChips=(e.tags&&e.tags.length)?`<div style="margin-top:4px">${e.tags.map(t=>`<span style="display:inline-block;background:var(--surface2);color:var(--muted);font-size:10px;padding:2px 8px;border-radius:100px;margin-right:4px;margin-top:2px;">#${t}</span>`).join('')}</div>`:'';
    const receiptBtn=e.receipt?`<button class="del-btn" onclick="viewReceipt(${oi})" title="View receipt">🧾</button>`:'';
    html+=`<div class="swipe-wrap" data-idx="${oi}"><div class="swipe-delete-bg">🗑️</div><div class="entry-row swipe-fg"><div class="entry-icon" style="background:${bg}">${isInc?'💰':emoji(e.category)}</div><div class="entry-info"><div class="entry-cat">${e.category} <span class="debt-entry-badge ${isInc?'badge-profit':'badge-debt'}" style="font-size:10px">${isInc?'Income':'Expense'}</span></div><div class="entry-meta">${e.month} ${e.year||''}${e.date?' • '+e.date:''}${payLabel}${debtTag}</div>${tagChips}</div><span class="entry-amt" style="color:${amtColor}">${isInc?'+':'−'}${fmt(e.amount)}</span>${receiptBtn}<button class="del-btn" onclick="openEditEntry(${oi})" title="Edit">✏️</button><button class="del-btn" onclick="delEntry(${oi})">✕</button></div></div>`;
  });
...
function showDateRangeReport(){
  ...
  const sorted=Object.entries(bycat).sort((a,b)=>b[1]-a[1]);
  const catRows=sorted.map(([c,amt])=>`<div style="display:flex;justify-content:space-between;font-size:12px;padding:4px 0"><span>${emoji(c)} ${c}</span><span style="color:var(--muted)">${fmt(amt)}</span></div>`).join('');
  resultEl.innerHTML=`
    <div style="display:flex;gap:8px;margin-bottom:10px">
...
function renderEditGrids(){
  ...
  const cats=getCats();let ch='';
  cats.forEach(c=>{ch+=`<div class="sel-btn${editCat===c?' sel':''}" onclick="selEditCat('${c}')">${emoji(c)} ${c}</div>`;});
...
async function saveEditEntry(){
  ...
  const e=d[editIndex];
  if(!e)return;
  e.month=editMonth;e.category=editCat;e.amount=amt;e.date=dateVal;
...
async function saveDebt(){
  ...
  const ts=Date.now();
  const newDebt={name,amount:amt,type:debtType,ts,cleared:false,dueDate:dueDate||null,interestRate:interestRate||null,installments:installments||null};
  const debts=getDebts();
  debts.push(newDebt);
  saveDebts(debts);
  ...
  const userId=await resolveSupabaseUserId();
  if(!userId)return;
  const{data:inserted,error}=await sb.from('debts').insert({
    user_id:userId,name,amount:amt,type:debtType,cleared:false,
    due_date:dueDate||null,interest_rate:interestRate||null,
    installments:installments||null,repaid:0,repay_log:[],entry_ts:ts
  }).select().single();
  if(error){console.error('Failed to sync debt to cloud:',error);showToast('⚠️ Cloud sync failed: '+(error.message||'unknown error'),'#e53935');return;}
  const d2=getDebts();
  const target=d2.find(x=>x.ts===ts&&x.name===name);
  if(target){target.supaId=String(inserted.id);saveDebts(d2);}
}
...
function checkDueAlerts(){
  ...
  upcoming.forEach(item=>{
    ...
    html+=`<div class="due-alert-item">
      <div class="due-alert-name">${item.isRecurring?'🔁 ':''}${item.name} — ${fmt(item.amount)}</div>
...
function renderManage(){
  ...
  cats.forEach((c,i)=>{
    ...
    html+=`<div class="manage-row"><span style="font-size:20px;width:32px;text-align:center">${emoji(c)}</span><span style="flex:1;font-size:15px">${c}</span>${isCustom?`<button onclick="resetCatColor('${c}')" title="Reset to default color" style="background:none;border:none;color:var(--muted);font-size:10px;margin-right:4px;cursor:pointer;">↺</button>`:''}<input type="color" value="${cl}" onchange="setCatColor('${c}',this.value)" title="Choose a custom color for ${c}" style="width:20px;height:20px;border:none;border-radius:50%;padding:0;margin-right:8px;cursor:pointer;background:none;"/><button class="del-btn" onclick="deleteCat(${i})">✕</button></div>`;
  });
  ...
  document.getElementById('cat-budget-list').innerHTML=cats.map(c=>`
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
      ...
      <div style="position:relative;width:110px">
        ...
        <input type="number" class="ti" value="${catBudgets[c]||''}" placeholder="No limit" onchange="setCatBudget('${c}',this.value)" style="padding:7px 7px 7px 22px;font-size:12px;width:100%"/>
      </div>
    </div>`).join('');
...
async function renderAdminMessages(){
  ...
  el.innerHTML=msgs.map(m=>{
    ...
    const subjectLine=m.subject?`<div style="font-size:13px;font-weight:700;margin-bottom:2px">${m.subject.replace(/</g,'&lt;')}</div>`:'';
    const meta=[m.category,m.priority?m.priority+' priority':null].filter(Boolean).join(' · ');
    return `<div style="background:var(--surface2);border-radius:12px;padding:12px;margin-bottom:8px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;gap:8px">
        <span style="font-size:13px;font-weight:700">${(m.username||'Unknown').replace(/</g,'&lt;')}</span>
      ...
      <div style="font-size:13px;line-height:1.6;margin-bottom:4px">${(m.message||'').replace(/</g,'&lt;')}</div>
      <div style="font-size:10px;color:var(--muted);margin-bottom:8px">${dt}</div>
      ${m.admin_reply?`<div style="padding:8px 10px;background:rgba(108,99,255,.12);border-radius:10px;font-size:12px;line-height:1.6;margin-bottom:8px"><b style="color:var(--accent)">Your reply:</b> ${String(m.admin_reply).replace(/</g,'&lt;')}</div>`:''}
...
async function renderMembers(){
  ...
  listEl.innerHTML=sorted.map(([u,p])=>{
    ...
    return `<div class="entry-row" style="flex-wrap:wrap">
      <div class="entry-icon" style="background:${p.isAdmin?'rgba(108,99,255,.2)':'rgba(0,208,132,.15)'}">${p.isAdmin?'👑':(p.name||u).charAt(0).toUpperCase()}</div>
      <div class="entry-info">
        <div class="entry-cat">${p.name||u} ${p.isAdmin?'<span class="debt-entry-badge badge-debt" style="font-size:10px">Admin</span>':''}</div>
        <div class="entry-meta">@${u} · ${entryCount} entries · ${debtCount} debt records · joined ${joined}</div>
      </div>
      ${isSelf?'':`<div style="display:flex;gap:6px;width:100%;margin-top:8px">
        <button class="pill-btn" style="font-size:11px" onclick="toggleAdminRole('${u}')">${p.isAdmin?'⬇ Remove Admin':'⬆ Make Admin'}</button>
        <button class="pill-btn" style="font-size:11px;color:var(--red)" onclick="deleteUserAccount('${u}')">🗑 Delete Account</button>
      </div>`}
    </div>`;
  }).join('');
}
...
async function deleteUserAccount(u){
  if(!confirm(`Permanently delete @${u}'s account? This removes their profile and cannot be undone. Their historical transactions/debts already synced to Supabase are not automatically erased.`))return;
  const users=getUsers();
  delete users[u];
  saveUsers(users);
...
function renderErrorLogs(){
  ...
  el.innerHTML=logs.map(l=>{
    ...
    return `<div style="background:var(--surface2);border-radius:12px;padding:10px 12px;margin-bottom:8px">
      <div style="display:flex;justify-content:space-between;gap:8px"><span style="font-size:12px;font-weight:700;color:var(--red)">${(l.message||'Error').replace(/</g,'&lt;')}</span><span style="font-size:10px;color:var(--muted);white-space:nowrap">${dt}</span></div>
      ${l.source?`<div style="font-size:11px;color:var(--muted);margin-top:4px">${String(l.source).replace(/</g,'&lt;')}${l.user?' · @'+l.user:''}</div>`:''}
    </div>`;
  }).join('');
}
...
async function doChangePw(){
  ...
  if(!await verifyAndMigratePassword(users,CU,old)){err.textContent='Current password is wrong.';return;}
  ...
  users[CU].password=await hashPassword(nw);saveUsers(users);
...
function exportCSV(){
  ...
  a.href=url;a.download='expenses_'+CU+'.csv';a.click();
...
function exportAccount(){
  ...
  const users=getUsers();
  const payload={version:2,exportedAt:new Date().toISOString(),appVersion:APP_VERSION,username:CU,profile:users[CU],data:getData(),cats:getCats(),debts:getDebts(),reminders:getReminders()};
  const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'});
  const a=document.createElement('a');const url=URL.createObjectURL(blob);a.href=url;
  a.download='expenses_backup_'+CU+'_v2.json';a.click();
  setTimeout(()=>URL.revokeObjectURL(url),1000);
...
function importAccount(e){
  ...
  r.onload=ev=>{
    try{
      ...
      showToast('✅ Backup imported locally. Log in again to refresh cloud data.');
...
async function loadSupportTickets(){
  ...
  if(!el)return;
  const userId=await resolveSupabaseUserId();
  if(!userId){el.innerHTML='<div style="font-size:12px;color:var(--muted)">Log in to see your tickets.</div>';return;}
    ...
    el.innerHTML=data.map(t=>{
      ...
      return `<div style="background:var(--surface2);border-radius:12px;padding:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px">
          <span style="font-size:13px;font-weight:700">${(t.subject||'(no subject)').replace(/</g,'&lt;')}</span>
          <span style="font-size:10px;font-weight:700;color:${sc};background:${sc}22;padding:2px 8px;border-radius:8px;white-space:nowrap">${t.status||'Open'}</span>
        </div>
        <div style="font-size:11px;color:var(--muted);margin-bottom:6px">${t.category||''}${t.priority?' · '+t.priority+' priority':''} · ${dt}</div>
        <div style="font-size:13px;line-height:1.6">${(t.message||'').replace(/</g,'&lt;')}</div>
        ${t.admin_reply?`<div style="margin-top:8px;padding:8px 10px;background:rgba(108,99,255,.12);border-radius:10px;font-size:12px;line-height:1.6"><b style="color:var(--accent)">Admin reply:</b> ${String(t.admin_reply).replace(/</g,'&lt;')}</div>`:''}
      </div>`;
    }).join('');
  }catch(e){
...
// ── FORGOT PASSWORD (identify by mobile or email, OTP sent by real email via EmailJS) ──
...
// Look up a user by either their registered mobile number or email address.
// Checks local storage first (fast path), then falls back to Supabase so this
// works even for accounts created on a different device.
async function findUserByIdentifierAsync(idRaw){
  const id=idRaw.trim().toLowerCase();
  const localUsers=getUsers();
  for(const uname in localUsers){
    const acc=localUsers[uname];
    if(acc.email&&acc.email.toLowerCase()===id) return{uname,row:null};
    if(acc.mobile&&acc.mobile===idRaw.trim()) return{uname,row:null};
  }
  const{data,error}=await sb.from('users').select('*').or(`email.eq.${id},mobile.eq.${idRaw.trim()}`).maybeSingle();
  if(error){console.error('Forgot-password lookup failed:',error);return null;}
  if(!data)return null;
  return{uname:data.username,row:data};
}
async function sendForgotOTP(){
  ...
  const found=await findUserByIdentifierAsync(idVal);
  if(!found){err.textContent='No account found with that mobile number or email.';return;}
  const uname=found.uname;
  const localUsers=getUsers();
  let acc=localUsers[uname];
  if(!acc){
    // Account exists in Supabase but not on this device — build a local record
    // from it so the rest of this flow (and resendOTP) work unchanged.
    const row=found.row;
    acc={name:row.name,email:row.email,mobile:row.mobile||null,password:row.password_hash,
      isAdmin:!!row.is_admin,require2FA:!!row.require2FA,createdAt:Date.now(),onboarded:true};
    localUsers[uname]=acc;
    saveUsers(localUsers);
  }
  if(!acc.email){err.textContent='This account has no email on file — please contact support.';return;}
  // Generate 6-digit OTP
  _fpOTP=String(Math.floor(100000+Math.random()*900000));
  _fpTargetUser=uname;
  const maskedEmail=acc.email.replace(/^(.{2}).+(@.+)$/,'$1***$2');
  document.getElementById('fp-mobile-display').textContent=maskedEmail;
  document.getElementById('fp-username-hint').textContent='Username: '+uname;
  ...
  await deliverOTPEmail(acc, uname);
}
async function deliverOTPEmail(acc, uname){
  const result=await sendEmailViaEmailJS(EMAILJS_CONFIG.otpTemplateId,{
    to_email:acc.email,
  ...
  if(result.ok){
    showToast('📧 OTP emailed to '+acc.email,'#00d084');
  } else if(result.reason==='send_failed'){
    // EmailJS IS configured but the actual send call failed — most likely a wrong Service/Template
    // ID, a template variable mismatch, a disabled email service, or the free-tier monthly quota.
    showToast('⚠️ Email send failed: '+result.detail,'#ff6584');
    setTimeout(()=>showToast('OTP: '+_fpOTP+' (shown here so you can still log in)','#6c63ff'),1800);
  } else {
    // EmailJS not configured yet — fall back to showing it on-screen so the app remains usable.
    showToast('⚠️ Email not configured — showing OTP here instead','#ffb300');
    setTimeout(()=>showToast('OTP: '+_fpOTP+' (set up EmailJS to send this by email)','#6c63ff'),1600);
  }
}
async function resendOTP(){
  if(!_fpTargetUser){return;}
  _fpOTP=String(Math.floor(100000+Math.random()*900000));
  const acc=getUsers()[_fpTargetUser];
  await deliverOTPEmail(acc, _fpTargetUser);
}
async function verifyForgotOTP(){
  ...
  // Update Supabase first — this is the actual source of truth doLogin() checks.
  const{error}=await sb.from('users').update({password_hash:newHash}).eq('username',_fpTargetUser);
  if(error){
    console.error('Password reset failed in Supabase:',error);
    err.textContent='Could not reset password — check your connection and try again.';
    return;
  }

  // Keep the local copy in sync too (used for require2FA and profile display).
  const users=getUsers();
  if(users[_fpTargetUser]){
    users[_fpTargetUser].password=newHash;
    saveUsers(users);
  }
...
function renderReminders(){
  ...
  el.innerHTML=rems.map(r=>{
    ...
    <div class="rem-row" style="${isPaused?'opacity:.5':''}">
      ...
      <div style="flex:1">
        <div style="font-size:14px;font-weight:600">${r.name}${isIncome?' <span style="font-size:10px;color:var(--green);font-weight:700">INCOME</span>':''}${isPaused?' <span style="font-size:10px;color:var(--muted);font-weight:700">PAUSED</span>':''}</div>
...
// Keyboard-friendly authentication: Enter submits the visible auth form.
['lu','lp','sn','su','se','sm','sp','lock-pw'].forEach(id=>{
  const el=document.getElementById(id);
  if(el)el.addEventListener('keydown',e=>{
    if(e.key!=='Enter')return;
    e.preventDefault();
    if(id==='lock-pw')doUnlock();
    else if(['sn','su','se','sm','sp'].includes(id))doSignup();
    else doLogin();
  });
});
...
async function resolveSupabaseUserId(){
  if(!CU)return null;
  const{data,error}=await sb.from('users').select('id').eq('username',CU).maybeSingle();
  if(error||!data){console.error('Could not resolve Supabase user id for',CU,error);return null;}
...
async function syncAndroidTransactions(){
  const cEl=document.getElementById('drawer-synccount');
  const userId = await resolveSupabaseUserId();
  if(!userId){
    if(cEl)cEl.textContent='Cloud sync: FAILED (no user id)';
    return;
  }
  ...
  if(error){
    ...
    if(cEl)cEl.textContent='Cloud sync: ERROR — '+(error.message||'see console');
    return;
  }
  ...
  if(cEl)cEl.textContent=`Cloud sync: ${(rows||[]).length} found in Supabase, ${added} new added, ${data.filter(e=>e.supaId).length} total tagged`;
...
function subscribeAndroidRealtime(userId){
  ...
  _androidRealtimeChannel=sb.channel('android-transactions-'+userId)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'transactions',filter:`user_id=eq.${userId}`},(payload)=>{
      ...
      const data=getData();
      if(data.some(e=>String(e.supaId)===String(txn.id)))return;
      data.push(androidTxnToEntry(txn));
      saveData(data);
      refreshVisibleTabsAfterSync();
...
async function syncDebts(){
  const userId=await resolveSupabaseUserId();
  if(!userId){showToast('⚠️ Debt sync skipped: user id not resolved','#e53935');return;}
  const{data:rows,error}=await sb.from('debts').select('*').eq('user_id',userId).order('entry_ts',{ascending:true});
  if(error){console.error('Error fetching debts:',error);showToast('⚠️ Debt sync failed: '+(error.message||'unknown error'),'#e53935');return;}
  const debts=getDebts();
...
function subscribeDebtsRealtime(userId){
  ...
  _debtsRealtimeChannel=sb.channel('debts-'+userId)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'debts',filter:`user_id=eq.${userId}`},(payload)=>{
      ...
      const debts=getDebts();
      if(debts.some(d=>String(d.supaId)===String(row.id)))return;
      debts.push(debtRowToLocal(row));
      saveDebts(debts);renderDebtPage();renderHome();
