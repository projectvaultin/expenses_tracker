...
// Canonical Supabase Auth user UUID used by UUID-based backend tables.
// Defined in the primary application script so it is globally available
// before any sync/debt/settings function can call it.
...
// Legacy bigint-user-id tables (categories, budgets, settings, reminders, support).
// These tables intentionally use public.users.id, not auth.users.id.
async function resolveLegacyUserId(){
  ...
  try{
    ...
    if(profileError || !profile) {
      console.error('Could not resolve legacy public.users.id',profileError);
      return null;
    }
    return profile.id;
  }catch(error){
    console.error('Could not resolve legacy public.users.id',error);
    return null;
  }
}
...
const EMAILJS_CONFIG={
  ...
  welcomeTemplateId:'template_hb3m4ei'     // One-time welcome email after signup
};
...
async function sendWelcomeEmail(email,name,username){
  ...
  return await sendEmailViaEmailJS(EMAILJS_CONFIG.welcomeTemplateId,{
    to_email:email,
    to_name:name||username||'there',
    username:username||'',
    app_url:'https://projectvaultin.github.io/expenses_tracker/',
    logo_url:'https://projectvaultin.github.io/expenses_tracker/icon-192.png'
  });
}
...
const SYNC_HEALTH_CHECK_MS=45000;
...
async function doSignup(){
  ...
  try{
  const users=getUsers();
  // Anonymous signup cannot directly SELECT public.users because RLS restricts profile reads.
  // Use a privacy-preserving RPC that returns availability booleans only.
  const { data: availability, error: availabilityError } = await sb.rpc(
    'check_signup_availability',
    { p_username: u, p_email: email, p_mobile: mobile || null }
  );
  if (availabilityError) {
    console.error('Signup availability check failed:', availabilityError);
    err.textContent='Could not check account availability. Please try again.';
    return;
  }
  const availabilityRow=Array.isArray(availability)?availability[0]:availability;
  if (!availabilityRow?.username_available) {
    err.textContent='Username already taken.';
    return;
  }
  if (!availabilityRow?.email_available) {
    err.textContent='An account already uses this email.';
    return;
  }
  if (mobile && !availabilityRow?.mobile_available) {
  ...
  if (!signUpData.session) {
    ...
    // Send the one-time welcome email even when Supabase requires email confirmation.
    // This is intentionally best-effort: a welcome-email failure must never block signup.
    const welcomeResult=await sendWelcomeEmail(email,name,u);
    if(!welcomeResult.ok)console.warn('Welcome email was not sent:',welcomeResult);
  ...
  // Send exactly one welcome email as part of the signup flow. Best-effort only so an
  // EmailJS issue cannot make an otherwise successful account creation appear to fail.
  const welcomeResult=await sendWelcomeEmail(email,name,finalUsername);
  if(!welcomeResult.ok)console.warn('Welcome email was not sent:',welcomeResult);
...
function startApp(profile){
  ...
  setTimeout(pvCheckPendingNotifications,1200);
  setTimeout(pvCheckPendingAppUpdate,1300);
...
if(supportCard){
  ...
  document.getElementById('admin-notification-section').style.display=isAdmin?'block':'none';
  const eventsDrawer=document.getElementById('drawer-events-media'); if(eventsDrawer)eventsDrawer.style.display=isAdmin?'flex':'none';
  ...
  setTimeout(()=>{ if(typeof pvLoadEventsMedia==='function') pvLoadEventsMedia(); if(typeof pvRenderHomeSponsoredMedia==='function') pvRenderHomeSponsoredMedia(); if(typeof pvOpenDisplay==='function') pvOpenDisplay(); },1300);
}

// ── NOTIFICATION CENTER ──
let _pvNotificationFilter='all';
let _pvNotificationCache=[];
function pvNotificationIcon(kind){return kind==='debt'?'🤝':kind==='announcement'?'📢':kind==='update'?'✨':'🔔';}
function pvNotificationKind(n){const k=String(n.kind||'').toLowerCase(); if(k.includes('debt')||k.includes('payment'))return 'debt'; if(k.includes('announcement'))return 'announcement'; if(k.includes('update')||k.includes('admin'))return 'update'; return 'all';}
async function pvLoadNotificationCenter(){
  const uid=await pvGetAuthId(); if(!uid)return [];
  const now=new Date().toISOString();
  const [nr,ar]=await Promise.all([
    sb.from('pv_notifications').select('*').eq('active',true).lte('scheduled_at',now).order('created_at',{ascending:false}).limit(100),
    sb.from('announcements').select('*').order('created_at',{ascending:false}).limit(100)
  ]);
  const notes=(nr.data||[]).map(n=>({...n,source:'notification',read:false,kind:pvNotificationKind(n)}));
  const anns=(ar.data||[]).map(a=>({id:'ann-'+a.id,title:'📢 Announcement',body:a.text,created_at:a.created_at,kind:'announcement',source:'announcement',announcement_id:a.id,read:false}));
  const all=[...notes,...anns].sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));
  const ids=notes.map(n=>n.id);
  if(ids.length){const {data:r}=await sb.from('pv_notification_receipts').select('notification_id,read_at').eq('user_id',uid).in('notification_id',ids);const rm=new Map((r||[]).map(x=>[String(x.notification_id),x]));notes.forEach(n=>{n.read=!!rm.get(String(n.id))?.read_at;});}
  const aid=anns.map(a=>Number(a.announcement_id)).filter(Number.isFinite);
  if(aid.length){const {data:r}=await sb.from('announcement_view_receipts').select('announcement_id,viewed_at').eq('user_id',uid).in('announcement_id',aid);const rm=new Map((r||[]).map(x=>[String(x.announcement_id),x]));anns.forEach(a=>{a.read=!!rm.get(String(a.announcement_id))?.viewed_at;});}
  _pvNotificationCache=all; return all;
}
async function pvRenderNotificationCenter(){const el=document.getElementById('pv-notifications-inbox');if(!el)return;el.innerHTML='<div class="empty"><div class="empty-icon">⏳</div><p>Loading notifications…</p></div>';const all=await pvLoadNotificationCenter();const rows=_pvNotificationFilter==='all'?all:all.filter(n=>pvNotificationKind(n)===_pvNotificationFilter);if(!rows.length){el.innerHTML='<div class="empty"><div class="empty-icon">🔕</div><p>No notifications here.</p></div>';return;}el.innerHTML=rows.map(n=>`<div class="pv-notif-row ${n.read?'read':'unread'}" onclick="pvOpenNotification('${escJsAttr(String(n.id))}','${escJsAttr(n.source)}','${escJsAttr(String(n.announcement_id||''))}')"><div class="pv-notif-icon">${pvNotificationIcon(pvNotificationKind(n))}</div><div style="flex:1;min-width:0"><div style="font-weight:800;font-size:13px">${escHtml(n.title)}</div><div style="font-size:12px;color:var(--muted);line-height:1.45;margin-top:3px">${escHtml(n.body)}</div><div style="font-size:10px;color:var(--muted);margin-top:5px">${new Date(n.created_at).toLocaleString('en-IN')}</div></div>${n.read?'':'<span class="pv-unread-dot"></span>'}</div>`).join('');await pvUpdateNotificationBadge();}
function pvSetNotificationFilter(f){_pvNotificationFilter=f;document.querySelectorAll('.notif-filter').forEach(b=>{b.classList.toggle('active',b.dataset.nfilter===f);});pvRenderNotificationCenter();}
async function pvOpenNotification(id,source,announcementId){const uid=await pvGetAuthId();if(!uid)return;if(source==='announcement'&&announcementId){await recordAnnouncementViewed(announcementId);}else if(id&&!String(id).startsWith('ann-')){await sb.from('pv_notification_receipts').upsert({notification_id:id,user_id:uid,delivered_at:new Date().toISOString(),read_at:new Date().toISOString()},{onConflict:'notification_id,user_id'});}await pvRenderNotificationCenter();}
async function pvMarkAllNotificationsRead(){const uid=await pvGetAuthId();if(!uid)return;const notes=await pvLoadNotificationCenter();for(const n of notes){if(n.read)continue;if(n.source==='announcement'&&n.announcement_id)await recordAnnouncementViewed(n.announcement_id);else if(n.id)await sb.from('pv_notification_receipts').upsert({notification_id:n.id,user_id:uid,delivered_at:new Date().toISOString(),read_at:new Date().toISOString()},{onConflict:'notification_id,user_id'});}await pvRenderNotificationCenter();showToast('✓ All notifications marked read');}
async function pvUpdateNotificationBadge(){const notes=await pvLoadNotificationCenter();const unread=notes.filter(n=>!n.read).length;const b=document.getElementById('notifications-tab-badge');if(b){b.textContent=unread>99?'99+':String(unread);b.style.display=unread?'inline-flex':'none';}const home=document.getElementById('pv-notifications-card');if(home){const top=notes.filter(n=>!n.read).slice(0,3);home.style.display=top.length?'block':'none';const list=document.getElementById('pv-notifications-list');if(list)list.innerHTML=top.map(n=>`<div style="padding:9px 0;border-top:1px solid var(--border)"><b>${pvNotificationIcon(pvNotificationKind(n))} ${escHtml(n.title)}</b><div style="font-size:11px;color:var(--muted);margin-top:3px">${escHtml(n.body)}</div></div>`).join('');}}
...
function switchTab(name,el){
  ...
  if(name==='debt'){renderDebtPage();if(typeof loadMoneySummary==='function')loadMoneySummary();}
  if(name==='manage')renderManage();
  if(name==='notifications')pvRenderNotificationCenter();
}

// ── HOW TO USE GUIDE ──
const PV_HOWTO={
  start:['🚀 Getting Started','Create an account with your name, username, email, mobile and password. Confirm your email if requested, then log in.','After login, ProjectVault loads your profile and dashboard. Add your first income or expense to start building your financial history.','A few accurate entries give you useful balances, categories, reports and spending insights.'],
  money:['💰 Income & Expenses','Use Add → choose Income or Expense → enter amount, date, category and payment mode → Save.','The entry is stored locally and synchronized to Supabase when cloud access is available.','Over time you get reliable balances, history, reports and year-to-date insights.'],
  accounts:['🏦 Bank Accounts','Open Manage/Accounts → add a bank account label, provider and only the last four digits. Never enter a full account number.','The account is stored with a safe masked identifier and synchronized to Android/Supabase.','You can keep transactions organized by account and safely manage multiple accounts.'],
  categories:['📊 Categories','Choose an existing category when recording a transaction, or manage your categories from Manage.','Category totals are reflected in the dashboard and reports.','Clear categories make it easy to see where your money is actually going.'],
  budgets:['🎯 Budgets','Set a monthly budget and optional category budgets from Manage.','ProjectVault compares your spending with your limits and shows progress/warnings on the dashboard.','Budget tracking helps you control overspending and understand monthly performance.'],
  recurring:['🔄 Recurring Payments','Add rent, EMI, subscriptions or recurring income. Choose weekly, monthly, quarterly or yearly frequency and the due day/date.','The reminder appears in upcoming items and can be marked Paid/Received, which logs the real transaction automatically.','You avoid forgetting recurring commitments and maintain consistent records month after month.'],
  debts:['🤝 Debts / Shared Money','Open Debts → add who you borrowed from or gave money to → enter amount and optional due date. For another ProjectVault member, search and select their username.','The debt tracks outstanding amount, repayment and status. Shared entries stay synchronized between the linked accounts.','You can see exactly who owes whom, what remains, and what has already been repaid.'],
  reports:['📈 Reports','Use History filters and the reporting tools, or open the monthly statement feature.','Reports summarize income, expenses, top categories, unused categories, budgets and useful spending insights.','You can understand patterns instead of looking at isolated transactions.'],
  notifications:['🔔 Notifications','Allow notifications when prompted. Recurring bills, repayment alerts, admin announcements and app updates can create alerts.','The WebApp shows persistent in-app notices; the Android app can receive background mobile notifications when configured and permitted.','You can notice important due dates without repeatedly checking every screen.'],
  events:['🎉 Events & Festivals','Administrators can create festivals, greetings and sponsored media with dates. Birthday information can be saved in the profile and visibility can be controlled.','On the relevant date, ProjectVault prioritizes the birthday/festival experience and scheduled media can be displayed automatically.','This gives members timely greetings and gives admins a controlled way to publish campaigns.'],
  sync:['🔄 Android Sync','Keep the Android app signed in and connected to the internet. Use Sync Status to inspect pending, syncing, synced and failed work.','WorkManager retries failed work and periodically pulls/pushes supported data.','Your Android and WebApp records stay aligned without requiring manual export/import every time.'],
  security:['🔐 Security & Settings','Use a strong password, enable optional 2FA, keep only the last four account digits, and review notification permissions.','Authentication is handled by Supabase Auth; the app does not use the old public password-hash flow.','Good security settings protect access while keeping your financial records organized.']
};
let _howToKey='start';
let _howToAutoTimer=null;
function howToKeys(){return Object.keys(PV_HOWTO);}
function openHowToUse(key='start'){
  const nav=document.getElementById('howto-nav'),content=document.getElementById('howto-content'); if(!nav||!content)return;
  nav.innerHTML=Object.entries(PV_HOWTO).map(([k,v],i)=>`<button class="pill-btn" data-howto="${k}" onclick="renderHowToUse('${k}')" style="padding:9px 8px;font-size:11px">${i+1}. ${v[0]}</button>`).join('');
  document.getElementById('how-to-use-modal').classList.add('open');
  _howToKey=key;
  renderHowToUse(key);
  clearTimeout(_howToAutoTimer);
  _howToAutoTimer=setTimeout(howToNext,7000);
}
function renderHowToUse(key){
  const v=PV_HOWTO[key]||PV_HOWTO.start,content=document.getElementById('howto-content'); if(!content)return;
  _howToKey=key;
  const keys=howToKeys(),idx=Math.max(0,keys.indexOf(key));
  document.querySelectorAll('[data-howto]').forEach(b=>{b.style.background=b.dataset.howto===key?'var(--accent)':'var(--surface2)';b.style.color=b.dataset.howto===key?'#fff':'var(--text)';});
  const pb=document.getElementById('howto-progress-bar'); if(pb)pb.style.width=((idx+1)/keys.length*100)+'%';
  const next=document.getElementById('howto-next-btn'); if(next)next.textContent=idx===keys.length-1?'Finish ✓':'Next ›';
  content.innerHTML=`<div style="border:1px solid rgba(108,99,255,.28);border-radius:18px;padding:16px;background:linear-gradient(180deg,rgba(108,99,255,.10),var(--surface));animation:fadeIn .35s ease">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px"><div style="font-size:32px">${v[0].slice(0,2)}</div><div><div style="font-size:17px;font-weight:800">${v[0].slice(2)}</div><div style="font-size:10px;color:var(--muted);margin-top:3px">STEP ${idx+1} OF ${keys.length} · WATCH & FOLLOW</div></div></div>
    <div style="display:flex;gap:7px;align-items:flex-start;margin:12px 0"><b style="min-width:24px;height:24px;border-radius:50%;background:var(--accent);color:#fff;text-align:center;padding-top:3px">1</b><div><b>What it is</b><div style="font-size:13px;line-height:1.65;color:var(--muted);margin-top:3px">${v[1]}</div></div></div>
    <div style="display:flex;gap:7px;align-items:flex-start;margin:12px 0"><b style="min-width:24px;height:24px;border-radius:50%;background:var(--accent);color:#fff;text-align:center;padding-top:3px">2</b><div><b>Do this</b><div style="font-size:13px;line-height:1.65;color:var(--muted);margin-top:3px">${v[2]}</div></div></div>
    <div style="display:flex;gap:7px;align-items:flex-start;margin:12px 0 0"><b style="min-width:24px;height:24px;border-radius:50%;background:var(--green);color:#fff;text-align:center;padding-top:3px">3</b><div><b>Then you get</b><div style="font-size:13px;line-height:1.65;color:var(--muted);margin-top:3px">${v[3]}</div></div></div>
  </div>`;
  clearTimeout(_howToAutoTimer);
  _howToAutoTimer=setTimeout(howToNext,7000);
}
function howToNext(){
  const keys=howToKeys(),idx=keys.indexOf(_howToKey);
  if(idx<keys.length-1)renderHowToUse(keys[idx+1]); else clearTimeout(_howToAutoTimer);
}
function howToPrev(){
  const keys=howToKeys(),idx=keys.indexOf(_howToKey);
  renderHowToUse(keys[Math.max(0,idx-1)]);
}
function closeHowToUse(){clearTimeout(_howToAutoTimer);document.getElementById('how-to-use-modal')?.classList.remove('open');}
...
async function pvApplySharedNetBalance(baseNet){
  try{
    const uid=await resolveSupabaseUserId(); if(!uid)return;
    const [{data:lent},{data:borrowed}]=await Promise.all([
      sb.from('shared_debts_with_computed_status').select('remaining,effective_status,status').eq('lender_id',uid),
      sb.from('shared_debts_with_computed_status').select('remaining,effective_status,status').eq('borrower_id',uid)
    ]);
    const owedToMe=(lent||[]).filter(d=>d.status!=='awaiting_confirmation'&&d.effective_status!=='cancelled').reduce((a,d)=>a+Number(d.remaining||0),0);
    const iOwe=(borrowed||[]).filter(d=>d.status!=='awaiting_confirmation'&&d.effective_status!=='cancelled').reduce((a,d)=>a+Number(d.remaining||0),0);
    const adjusted=baseNet+owedToMe-iOwe;
    const el=document.getElementById('grand-total'); if(!el)return;
    animateNet(el,adjusted); el.style.color=adjusted<0?'var(--red)':adjusted>0?'var(--green)':'var(--text)';
    const period=document.getElementById('total-period'); if(period)period.textContent=filterMonth==='All'?'Income − Expenses + Active Shared Money':filterMonth+' + active shared money';
  }catch(e){console.warn('shared net balance update failed',e);}
}
function renderHome(){
  maybeShowNotifPrompt();
  checkForAppUpdates();
  ...
  pvApplySharedNetBalance(net);
  const hfl=document.getElementById('home-filter-label'); if(hfl)hfl.textContent=filterMonth==='All'?'Showing all months':'Showing '+filterMonth;
...
function renderAddForm(){
  ...
  if(receiptWrap&&!pendingReceipt){
    receiptWrap.innerHTML=`<div id="receipt-add-btn-dynamic" onclick="document.getElementById('receipt-file-dynamic').click()" style="display:flex;align-items:center;gap:8px;background:var(--surface2);border:1px dashed var(--border);border-radius:14px;padding:12px;cursor:pointer;font-size:13px;color:var(--muted)"><span style="font-size:18px">📎</span> Attach a photo of the receipt</div><input type="file" id="receipt-file-dynamic" accept="image/*" style="display:none" onchange="handleReceiptPhoto(event)"/>`;
  }
}
...
function showNameSuggestions(val){
  ...
  list.innerHTML=matches.map(n=>{
    ...
    return `<div class="ac-item" onclick="pickAcName('${escJsAttr(n)}')"><span>${escHtml(n)}</span><span class="ac-badge debt-entry-badge ${badgeClass}">${badgeText}</span></div>`;
  }).join('');
...
async function saveDebt(){
  const selected=_selectedDebtUser;
  const name=selected ? (selected.name||selected.username||'ProjectVault user') : (document.getElementById('debt-name')?.value||'').trim();
  ...
  const errEl=document.getElementById('shared-debt-save-error');
  if(errEl)errEl.textContent='';
  if(!selected){showToast('Select a registered ProjectVault user','#ffb300');return;}
  if(!amt||amt<=0){showToast('Enter a valid amount','#ffb300');return;}
  if(!selDebtType){showToast('Select type','#ffb300');return;}

  const userId=await resolveSupabaseUserId();
  if(!userId){showToast('Could not resolve your account','#ffb300');return;}
  const otherId=selected.auth_user_id;
  if(!otherId||otherId===userId){showToast('Choose another ProjectVault user','#ffb300');return;}

  // I Borrowed: selected person is lender. I Gave: current user is lender.
  const lenderId=selDebtType==='profit' ? userId : otherId;
  const borrowerId=selDebtType==='profit' ? otherId : userId;
  const {data:shared,error}=await sb.from('shared_debts').insert({
    lender_id:lenderId, borrower_id:borrowerId, amount:amt, reason:reason||null, due_date:dueDate||null,
    status:selDebtType==='profit'?'awaiting_confirmation':'pending'
  }).select().single();
  if(error){
    console.error('Failed to save shared debt:',error);
    if(errEl)errEl.textContent=error.message||'Failed to save shared entry';
    showToast('⚠️ Shared entry failed','#e53935');
    return;
  }

  // Keep a local mirror so the existing debt dashboard also reflects the entry.
  const ts=Date.now();
  // Money given to a friend is not added to the local debt list until the
  // recipient confirms it. The sender gets a pending shared record only.
  if(selDebtType!=='profit'){
    const local={name,amount:amt,type:selDebtType,ts,cleared:false,dueDate:dueDate||null,interestRate:interestRate||null,installments:installments||null,repaid:0,sharedDebtId:shared.id,sharedUserId:otherId};
    const debts=getDebts();debts.push(local);saveDebts(debts);
  }
  const ids=['shared-debt-user-search','debt-amount','debt-due-date','debt-interest','debt-installments','debt-reason'];
  ...
  renderDebtPage();renderHome();loadMoneySummary();
  showToast('✅ Shared entry saved for both users');
}
  ...
  sorted.forEach(d=>{
    ...
    html+=`<div class="entry-row" style="${fullyCleared?'opacity:.55':''}">
      ...
      <div class="entry-info" style="flex:1">
        <div class="entry-cat">${escHtml(d.name)} ${repaidBadge}</div>
...
function renderManage(){
  ...
  if(profile){
    ...
    else{pavBox.innerHTML=`<span id="profile-av-ph-dynamic">${(profile.name||'U').charAt(0).toUpperCase()}</span>`;}
...
// ── PERSISTENT SUPABASE NOTIFICATIONS ─────────────────────────────────────
async function pvPublishNotification(){
  const title=document.getElementById('pv-notif-title-inp')?.value.trim();
  const body=document.getElementById('pv-notif-body-inp')?.value.trim();
  if(!title||!body){showToast('Enter notification title and message','#ffb300');return;}
  const uid=await pvGetAuthId();
  if(!uid){showToast('Admin session unavailable','#ffb300');return;}
  const {error}=await sb.from('pv_notifications').insert({title,body,kind:'admin',scheduled_at:new Date().toISOString(),active:true,created_by:uid});
  if(error){console.error(error);showToast('Notification failed: '+error.message,'#e53935');return;}
  document.getElementById('pv-notif-title-inp').value='';document.getElementById('pv-notif-body-inp').value='';
  showToast('🔔 Notification queued for all users!');
}
async function pvCheckPendingNotifications(){
  const uid=await pvGetAuthId();if(!uid)return;
  try{
    const now=new Date().toISOString();
    const {data:notes,error}=await sb.from('pv_notifications').select('*').eq('active',true).lte('scheduled_at',now).order('created_at',{ascending:false}).limit(20);
    if(error)throw error;
    const ids=(notes||[]).map(n=>n.id);if(!ids.length)return;
    const {data:receipts}=await sb.from('pv_notification_receipts').select('notification_id,delivered_at,read_at').eq('user_id',uid).in('notification_id',ids);
    const rm=new Map((receipts||[]).map(r=>[String(r.notification_id),r]));
    const pending=[];
    for(const n of (notes||[])){
      const r=rm.get(String(n.id));
      if(r?.read_at)continue;
      if(!r){const {error:ie}=await sb.from('pv_notification_receipts').insert({notification_id:n.id,user_id:uid,delivered_at:new Date().toISOString()});if(ie&&ie.code!=='23505')console.warn('notification receipt:',ie);}
      pending.push(n);
    }
    const card=document.getElementById('pv-notifications-card'),list=document.getElementById('pv-notifications-list');
    if(!pending.length||!card||!list){if(card)card.style.display='none';return;}
    card.style.display='block';
    list.innerHTML=pending.map(n=>`<div style="padding:10px 0;border-top:1px solid var(--border)"><div style="font-weight:800;font-size:13px">${escHtml(n.title)}</div><div style="font-size:12px;color:var(--muted);line-height:1.6;margin:4px 0 8px;white-space:pre-wrap">${escHtml(n.body)}</div><button class="pill-btn" onclick="pvMarkNotificationRead('${escJsAttr(n.id)}')">✓ Mark as read</button></div>`).join('');
    if('Notification' in window&&Notification.permission==='granted'){
      for(const n of pending){const r=rm.get(String(n.id));if(!r){try{new Notification('ProjectVault — '+n.title,{body:n.body,tag:'pv-notification-'+n.id});}catch(e){}}}
    }
  }catch(e){console.error('Pending notification check failed:',e);}
}
async function pvMarkNotificationRead(id){
  const uid=await pvGetAuthId();if(!uid)return;
  const {error}=await sb.from('pv_notification_receipts').upsert({notification_id:id,user_id:uid,delivered_at:new Date().toISOString(),read_at:new Date().toISOString()},{onConflict:'notification_id,user_id'});
  if(error){showToast('Could not mark as read','#ffb300');return;}
  showToast('✓ Notification read');await pvCheckPendingNotifications();
}
...
async function hasViewedAnnouncement(id){
  if(!id||String(id).startsWith('local-')) return false;
  const uid=await pvGetAuthId(); if(!uid)return false;
  try{
    const{data,error}=await sb.from('announcement_view_receipts').select('announcement_id').eq('announcement_id',id).eq('user_id',uid).maybeSingle();
    return !error&&!!data;
  }catch(e){return false;}
}
async function recordAnnouncementViewed(id){
  if(!id||String(id).startsWith('local-'))return;
  const uid=await pvGetAuthId();if(!uid)return;
  try{await sb.from('announcement_view_receipts').upsert({announcement_id:Number(id),user_id:uid,viewed_at:new Date().toISOString()},{onConflict:'announcement_id,user_id'});}catch(e){console.warn('announcement view receipt failed',e);}
}
...
async function checkAnnouncement(){
  ...
  if(await hasViewedAnnouncement(notice.id)){localStorage.setItem('exp_announcement_dismissed',notice.id);banner.style.display='none';return;}
  document.getElementById('announcement-text').textContent=notice.text;
  banner.style.display='flex';
}
async function dismissAnnouncement(){
  const notice=await getAnnouncement();
  if(notice){localStorage.setItem('exp_announcement_dismissed',notice.id);await recordAnnouncementViewed(notice.id);}
...
async function pushUpdateNotice(){
  ...
  try{
    ...
    const{error}=await sb.from('app_updates').insert({
      title:title||`Version ${ver}`,version:ver,description:notes,active:true,scheduled_at:new Date().toISOString(),published_by_user_id:(await pvGetAuthId())
    });
    if(error)throw error;
  }catch(e){
...
async function loadSupportTickets(){
  ...
  async function pvGetAuthId(){try{return (await sb.auth.getUser()).data?.user?.id||null;}catch(e){return null;}}
async function pvCheckPendingAppUpdate(){
  const uid=await pvGetAuthId(); if(!uid)return;
  try{
    const now=new Date().toISOString();
    const {data,error}=await sb.from('app_updates').select('*').eq('active',true).lte('scheduled_at',now).order('created_at',{ascending:false}).limit(1).maybeSingle();
    if(error||!data)return;
    const {data:receipt}=await sb.from('pv_app_update_receipts').select('read_at,delivered_at').eq('update_id',data.id).eq('user_id',uid).maybeSingle();
    if(receipt?.read_at)return;
    if(!receipt){await sb.from('pv_app_update_receipts').insert({update_id:data.id,user_id:uid,delivered_at:new Date().toISOString()});
      if('Notification' in window&&Notification.permission==='granted')try{new Notification('ProjectVault — '+(data.title||'App Update'),{body:data.description||('Version '+data.version+' is available'),tag:'pv-update-'+data.id});}catch(e){}
    }
    window.latestUpdate=data;
    const banner=document.getElementById('update-banner'); if(banner)banner.style.display='flex';
    const txt=document.getElementById('update-version-text'); if(txt)txt.textContent=`${data.version} • ${data.title||'New update available'}`;
  }catch(e){console.error('Update delivery check failed:',e);}
}
async function checkForAppUpdates(){return pvCheckPendingAppUpdate();}
async function showUpdateModal(){
  if(!window.latestUpdate)return;
  const update=window.latestUpdate;
  document.getElementById('update-modal-content').innerHTML=`<div style="font-size:18px;font-weight:700;margin-bottom:10px">🚀 ${escHtml(update.title||'App Update')}</div><div style="font-size:13px;color:var(--accent);margin-bottom:12px">Version ${escHtml(update.version||'')}</div><div style="line-height:1.8;white-space:pre-wrap">${escHtml(update.description||'')}</div>`;
  document.getElementById('update-modal').classList.add('open');
}
async function dismissUpdate(){
  if(window.latestUpdate){
    const uid=await pvGetAuthId();
    if(uid)await sb.from('pv_app_update_receipts').upsert({update_id:window.latestUpdate.id,user_id:uid,delivered_at:new Date().toISOString(),read_at:new Date().toISOString()},{onConflict:'update_id,user_id'});
  }
  document.getElementById('update-modal').classList.remove('open');
  const b=document.getElementById('update-banner');if(b)b.style.display='none';
}
...
async function findAuthEmailByIdentifier(idRaw){
  ...
  const{data,error}=await sb.rpc('get_auth_email_by_mobile',{p_mobile:idRaw.trim()});
...
async function checkAndSendWeeklyDigest(){
  ...
  const reportInsights=buildReportInsights(data,rangeLabel,now.getFullYear(),{totalIncome,totalExpense,avgDaily:(totalExpense/7),count:data.length});
  const result=await sendStatementEmail(acc,CU,rangeLabel,now.getFullYear(),{totalIncome,totalExpense,avgDaily:(totalExpense/7),count:data.length},yStats,catBreakdown,reportInsights);
...
async function checkAndSendAnnualSummary(){
  ...
  const reportInsights=buildReportInsights(yearData,'Full Year Summary',prevYear,{totalIncome:yearIncome,totalExpense:yStats.totalExpense,avgDaily:yStats.totalExpense/365,count:yearData.length});
  const result=await sendStatementEmail(acc,CU,'Full Year Summary',prevYear,{totalIncome:yearIncome,totalExpense:yStats.totalExpense,avgDaily:yStats.totalExpense/365,count:yearData.length},yStats,catBreakdown,reportInsights);
...
async function checkAndSendMonthlyStatement(){
  ...
  if(acc.lastStatementSent===key)return; // fast local guard
  try{
    const uid=await pvGetAuthId();
    if(uid){
      const {data:receipt}=await sb.from('pv_statement_receipts').select('period_key').eq('user_id',uid).eq('period_key',key).maybeSingle();
      if(receipt){acc.lastStatementSent=key;saveUsers(users);return;}
    }
  }catch(e){console.warn('Statement receipt lookup failed; continuing with local guard',e);}
  const data=getData();
  ...
  try{
    ...
    const reportInsights=buildReportInsights(periodData,monthName,year,mStats);
    const result=await sendStatementEmail(acc,CU,monthName,year,mStats,yStats,catBreakdown,reportInsights);
    if(result.ok){
      acc.lastStatementSent=key;
      saveUsers(users);
      try{const uid=await pvGetAuthId();if(uid)await sb.from('pv_statement_receipts').upsert({user_id:uid,period_key:key,sent_at:new Date().toISOString()},{onConflict:'user_id,period_key'});}catch(e){console.warn('Statement receipt save failed',e);}
...
// Builds the extra insights used by the enhanced monthly report.
// All calculations are derived from the same local transaction/budget data used by the app.
function buildReportInsights(periodData,monthName,year,mStats){
  const rows=Array.isArray(periodData)?periodData:[];
  const expenses=rows.filter(e=>e.type!=='income'&&Number(e.amount)>0);
  const bycat={};
  expenses.forEach(e=>{
  ...
  const sorted=Object.entries(bycat).sort((a,b)=>b[1]-a[1]);
  const top=sorted[0];
  const topCategory=top?`${emoji(top[0])} ${top[0]} — ${fmt(top[1])}`:'No expense category recorded.';

  const configuredCats=[...new Set(getCats().filter(c=>c&&c!=='Salary'))];
  const noSpend=configuredCats.filter(cat=>!Object.prototype.hasOwnProperty.call(bycat,cat));
  const noSpendCategories=noSpend.length?noSpend.join(', '):'All configured expense categories had spending.';

  const highest=expenses.reduce((max,e)=>Number(e.amount)>Number(max?.amount||0)?e:max,null);
  const highestExpense=highest?`${highest.category||'Other'} — ${fmt(Number(highest.amount||0))}`:'No individual expense recorded.';

  const catBudgets=getCatBudgets();
  const budgetRows=Object.entries(catBudgets)
    .map(([category,budget])=>({category,budget:Number(budget)||0,spent:Number(bycat[category]||0)}))
    .filter(x=>x.budget>0);
  const budgetTotal=budgetRows.reduce((s,x)=>s+x.budget,0);
  const budgetSpent=budgetRows.reduce((s,x)=>s+x.spent,0);
  const overBudget=budgetRows.filter(x=>x.spent>x.budget);
  const globalBudget=getBudgetLimit();

  let budgetStatus='No category or monthly budget is set.';
  if(globalBudget>0){
    const pct=(mStats.totalExpense/globalBudget)*100;
    const state=mStats.totalExpense>globalBudget?'over budget':mStats.totalExpense===globalBudget?'at the budget limit':'within budget';
    budgetStatus=`Monthly budget: ${fmt(globalBudget)} | Spent: ${fmt(mStats.totalExpense)} | ${Math.round(pct)}% used — ${state}.`;
    if(getRolloverEnabled()){
      const rollover=getRolloverAmount();
      if(rollover>0)budgetStatus+=` Rollover available: ${fmt(rollover)}.`;
    }
  }
  if(budgetTotal>0){
    const pct=(budgetSpent/budgetTotal)*100;
    budgetStatus+=` Category budgets: ${fmt(budgetSpent)} of ${fmt(budgetTotal)} used (${Math.round(pct)}%).`;
    if(overBudget.length)budgetStatus+=` ${overBudget.length} ${overBudget.length===1?'category is':'categories are'} over budget: ${overBudget.map(x=>x.category).join(', ')}.`;
    else budgetStatus+=' No category budget was exceeded.';
  }

  const remaining=(mStats.totalIncome||0)-(mStats.totalExpense||0);
  let spendingInsight='';
  if(!expenses.length){
    spendingInsight='No expenses were recorded during this period.';
  }else if(mStats.totalIncome>0&&remaining<0){
    spendingInsight=`Expenses exceeded income by ${fmt(Math.abs(remaining))}. Consider reviewing ${top?top[0]:'your largest spending areas'} next month.`;
  }else if(mStats.totalIncome>0&&remaining>0){
    const rate=(mStats.totalExpense/mStats.totalIncome)*100;
    spendingInsight=`You spent ${Math.round(rate)}% of your recorded income this period and retained ${fmt(remaining)}.`;
  }else if(top){
    spendingInsight=`${top[0]} was your largest spending area at ${fmt(top[1])}.`;
  }else{
    spendingInsight='Review your expense categories regularly to keep spending on track.';
  }

  return {
    top_category:topCategory,
    no_spend_categories:noSpendCategories,
    transaction_count:String(rows.length),
...
async function sendStatementEmail(acc,uname,monthName,year,mStats,yStats,catSlots,reportInsights){
  const remaining=(mStats.totalIncome||0)-(mStats.totalExpense||0);
  ...
  return await sendEmailViaEmailJS(EMAILJS_CONFIG.statementTemplateId,{
    ...
    top_category:insights.top_category||'Not available',
    no_spend_categories:insights.no_spend_categories||'Not available',
    transaction_count:insights.transaction_count||String(mStats.count||0),
    highest_expense:insights.highest_expense||'Not available',
    budget_status:insights.budget_status||'No budget information available.',
    spending_insight:insights.spending_insight||'Review your spending regularly.',
    ...slots,
...
function saveReminders(r){localStorage.setItem(uKey('reminders'),JSON.stringify(r));}
  ...
  if(!CU||!sb)return false;
  const userId=await resolveLegacyUserId();
  const authId=(await sb.auth.getUser()).data?.user?.id||null;
  if(userId===null||userId===undefined||!authId)return false;
  const payload={id:Number(r.id),user_id:Number(userId),auth_user_id:authId,name:r.name||null,amount:Number(r.amount),day:Number(r.day),month:r.month===null||r.month===undefined?null:Number(r.month),type:r.type||'expense',frequency:r.frequency||'monthly',active:r.active!==false,paid:r.paid||{}};
  const {error}=await sb.from('reminders').upsert(payload,{onConflict:'id'});
  if(error){console.error('Reminder upload failed:',error);return false;}
  return true;
}
async function deleteReminderCloud(id){
  ...
  const {error}=await sb.from('reminders').delete().eq('id',Number(id)).eq('user_id',Number(userId));
...
async function syncReminders(){
  ...
  const userId=await resolveLegacyUserId();
  if(userId===null||userId===undefined)return false;
  const {data,error}=await sb.from('reminders').select('*').eq('user_id',Number(userId)).order('updated_at',{ascending:true});
  if(error){console.error('Reminders sync failed:',error);return false;}
  const rems=(data||[]).map(row=>({id:Number(row.id),name:row.name||'',amount:Number(row.amount),day:Number(row.day),month:row.month===null?null:Number(row.month),active:row.active!==false,paid:row.paid||{},type:row.type||'expense',frequency:row.frequency||'monthly'}));
  saveReminders(rems);
  return true;
}
...
async function pvRefreshAppCache(){
  try{
    if('caches' in window){const keys=await caches.keys();await Promise.all(keys.filter(k=>k!=='expensestracker-cache-v10').map(k=>caches.delete(k)));}
    if(navigator.serviceWorker?.controller){navigator.serviceWorker.controller.postMessage({type:'PV_REFRESH_CACHE'});}
    showToast('🔄 Refreshing ProjectVault…');
    setTimeout(()=>location.reload(),350);
  }catch(e){console.warn('cache refresh failed',e);location.reload();}
}
...
let _selectedDebtUser = null;
window._debtSearchResults = [];
let _debtSearchTimer=null;
let _debtSearchRequest=0;

async function searchDebtUser(query){
  const raw=String(query||'').trim();
  const q=raw.replace(/^@+/, '').trim();
  const results=document.getElementById('shared-debt-user-results');
  if(!results)return;
  if(_debtSearchTimer){clearTimeout(_debtSearchTimer);_debtSearchTimer=null;}
  window._debtSearchResults=[];
  if(!q){results.innerHTML='';results.classList.remove('open');return;}
  if(q.length<2){results.innerHTML='<div style="opacity:.6;font-size:12px;padding:8px">Type at least 2 characters…</div>';results.classList.add('open');return;}
  const requestId=++_debtSearchRequest;
  results.innerHTML='<div style="opacity:.6;font-size:12px;padding:8px">🔎 Searching ProjectVault users…</div>';results.classList.add('open');
  _debtSearchTimer=setTimeout(async()=>{
    try{
      if(!sb){throw new Error('Supabase client is not ready');}
      const {data,error}=await sb.rpc('search_users_for_shared_money',{p_query:q});
      if(requestId!==_debtSearchRequest)return;
      if(error)throw error;
      const rows=(Array.isArray(data)?data:[]).filter(u=>u&&u.auth_user_id);
      window._debtSearchResults=rows;
      if(!rows.length){
        results.innerHTML='<div style="opacity:.6;font-size:12px;padding:8px">No ProjectVault user found for “'+escHtml(q)+'”</div>';results.classList.add('open');
        return;
      }
      results.innerHTML=rows.map((u,i)=>{
        const displayName=String(u.name||'').trim()||String(u.username||'').trim()||'ProjectVault user';
        const username=String(u.username||'').trim();
        return `<button type="button" class="pill-btn" style="display:block;width:100%;text-align:left;margin-top:5px;background:var(--surface2);border:1px solid var(--border);cursor:pointer;padding:10px 12px" onclick="selectDebtUser(${i})">
          <b>${escHtml(displayName)}</b>
          ${username?`<span style="display:block;opacity:.7;font-size:11px;margin-top:3px">@${escHtml(username)}</span>`:''}
        </button>`;
      }).join('');
      ...
      pushErrorLog({message:'Debt user search failed: '+(e.message||e),source:'searchDebtUser'});
      results.innerHTML='<div style="color:var(--red);font-size:12px;padding:8px">Search failed — '+escHtml(e.message||'database error')+'</div>';results.classList.add('open');
    }
  },250);
}

function selectDebtUser(index){
  const u=window._debtSearchResults[index];
  if(!u)return;
  _selectedDebtUser=u;
  const input=document.getElementById('shared-debt-user-search');
  const results=document.getElementById('shared-debt-user-results');
  const selected=document.getElementById('shared-debt-selected');
  if(input)input.value='';
  if(results){results.innerHTML='';results.classList.remove('open');}
  if(selected)selected.innerHTML=`
    <div style="padding:8px 10px;border-radius:10px;background:rgba(0,208,132,.10);border:1px solid rgba(0,208,132,.25)">
      <b>${escHtml(u.name||u.username)}</b>
      <span style="opacity:.65;font-size:11px"> @${escHtml(u.username)}</span>
      <button type="button" class="pill-btn" style="float:right;font-size:10px" onclick="clearDebtUser()">Change</button>
    </div>`;
}

function clearDebtUser(){
  _selectedDebtUser=null;
  const selected=document.getElementById('shared-debt-selected');
  if(selected)selected.innerHTML='';
}
...
async function submitSharedDebt(){
  ...
  const borrowerId=(_selectedDebtUser&&_selectedDebtUser.auth_user_id)||document.getElementById('give-money-modal').dataset.borrowerId;
  const userId=await resolveSupabaseUserId();
...
async function loadMoneySummary(){
  const userId=await resolveSupabaseUserId();
  if(userId===null||userId===undefined)return;
  const summaryEls=[document.getElementById('social-money-summary'),document.getElementById('social-money-summary-modal')].filter(Boolean);
  const lentEls=[document.getElementById('social-money-lent'),document.getElementById('social-money-lent-modal')].filter(Boolean);
  const borrowedEls=[document.getElementById('social-money-borrowed'),document.getElementById('social-money-borrowed-modal')].filter(Boolean);
  const setHtml=(els,html)=>els.forEach(el=>{el.innerHTML=html;});
  setHtml(summaryEls,'<div style="opacity:.6">Loading…</div>');
  ...
  if(e1||e2){
    ...
    setHtml(summaryEls,'<div style="color:var(--red)">Failed to load — check Error Logs</div>');
    return;
  }

  const pendingConfirmations=(borrowed||[]).filter(d=>d.status==='awaiting_confirmation');
  const confirmCard=document.getElementById('shared-money-confirmations');
  const confirmList=document.getElementById('shared-money-confirmation-list');
  if(confirmCard&&confirmList){
    confirmCard.style.display=pendingConfirmations.length?'block':'none';
    if(pendingConfirmations.length){
      const ids= pendingConfirmations.map(d=>d.lender_id);
      const senderNames=await fetchProfileNames(ids);
      confirmList.innerHTML=pendingConfirmations.map(d=>`<div style="padding:12px 0;border-bottom:1px solid var(--border)">
        <div style="font-weight:700">${escHtml(senderNames[d.lender_id]||'ProjectVault user')} sent you ₹${Number(d.amount).toFixed(2)}</div>
        <div style="font-size:12px;color:var(--muted);margin:4px 0 10px">${d.reason?escHtml(d.reason)+' · ':''}${d.due_date?'Due '+escHtml(d.due_date):'No due date'}</div>
        <div style="display:flex;gap:8px"><button class="save-btn" style="flex:1" onclick="confirmIncomingSharedDebt('${d.id}')">✅ Yes, this is correct</button><button class="save-btn" style="flex:1;background:var(--surface2);color:var(--red)" onclick="rejectIncomingSharedDebt('${d.id}')">❌ Not correct</button></div>
      </div>`).join('');
    }
  }

  // Keep accepted friend debts in the same main Debts list as manually-created debts.
  // Reconcile every confirmed/active borrowed shared debt into the local mirror so the
  // entry cannot disappear simply because the Shared Money card was refreshed.
  const localDebts=getDebts();
  let localChanged=false;
  const borrowedLenderIds=(borrowed||[]).filter(d=>d.status!=='awaiting_confirmation'&&d.effective_status!=='cancelled').map(d=>d.lender_id);
  const borrowedNameMap=await fetchProfileNames(borrowedLenderIds);
  (borrowed||[]).filter(d=>d.status!=='awaiting_confirmation'&&d.effective_status!=='cancelled').forEach(row=>{
    let local=localDebts.find(x=>String(x.sharedDebtId)===String(row.id));
    if(!local){
      local={name:borrowedNameMap?.[row.lender_id]||row.reason||'Money received',amount:Number(row.amount),type:'debt',ts:new Date(row.created_at).getTime(),cleared:row.effective_status==='paid',dueDate:row.due_date||null,repaid:Math.max(0,Number(row.amount)-Number(row.remaining||0)),repayLog:[],sharedDebtId:row.id,sharedUserId:row.lender_id};
      localDebts.push(local);localChanged=true;
    }else{
      const nextRepaid=Math.max(0,Number(row.amount)-Number(row.remaining||0));
      if(local.amount!==Number(row.amount)||local.repaid!==nextRepaid||local.cleared!==(row.effective_status==='paid')){local.amount=Number(row.amount);local.repaid=nextRepaid;local.cleared=row.effective_status==='paid';localChanged=true;}
      if(row.due_date&&local.dueDate!==row.due_date){local.dueDate=row.due_date;localChanged=true;}
    }
  });
  if(localChanged){saveDebts(localDebts);renderDebtPage();renderHome();}

  const activeLent=(lent||[]).filter(d=>d.effective_status!=='cancelled'&&d.status!=='awaiting_confirmation');
  const activeBorrowed=(borrowed||[]).filter(d=>d.effective_status!=='cancelled'&&d.status!=='awaiting_confirmation');
  const owedToMe=activeLent.reduce((s,d)=>s+Number(d.remaining),0);
  const iOwe=activeBorrowed.reduce((s,d)=>s+Number(d.remaining),0);
  ...
  setHtml(lentEls,(activeLent.length) ? activeLent.map(d=>renderDebtRow(d,'borrower_id',false)).join('') : '<div style="opacity:.6">Nothing given yet</div>');
  setHtml(borrowedEls,(activeBorrowed.length) ? activeBorrowed.map(d=>renderDebtRow(d,'lender_id',true)).join('') : '<div style="opacity:.6">Nothing borrowed</div>');
}

async function confirmIncomingSharedDebt(debtId){
  const {data,error}=await sb.rpc('confirm_shared_debt',{p_shared_debt_id:debtId});
  if(error||data!==true){showToast('⚠️ Could not confirm this amount','#e53935');return;}
  // It now becomes an active local debt entry only after the recipient confirms.
  const {data:row}=await sb.from('shared_debts').select('*').eq('id',debtId).maybeSingle();
  if(row){
    const senderNames=await fetchProfileNames([row.lender_id]);
    const userId=await resolveSupabaseUserId();
    const debts=getDebts();
    const existing=debts.find(d=>String(d.sharedDebtId)===String(row.id));
    if(existing){
      existing.name=senderNames?.[row.lender_id]||existing.name||row.reason||'Money received';
      existing.amount=Number(row.amount); existing.type='debt'; existing.cleared=false;
      existing.dueDate=row.due_date||existing.dueDate||null; existing.sharedUserId=row.lender_id;
      saveDebts(debts);
    }else{
      debts.push({name:senderNames?.[row.lender_id]||row.reason||'Money received',amount:Number(row.amount),type:'debt',ts:new Date(row.created_at).getTime(),cleared:false,dueDate:row.due_date||null,repaid:0,repayLog:[],sharedDebtId:row.id,sharedUserId:row.lender_id});
      saveDebts(debts);
    }
  }
  renderDebtPage();renderHome();loadMoneySummary();showToast('✅ Confirmed and added to Debts');
}
async function rejectIncomingSharedDebt(debtId){
  const {data,error}=await sb.rpc('reject_shared_debt',{p_shared_debt_id:debtId});
  if(error||data!==true){showToast('⚠️ Could not reject this amount','#e53935');return;}
  loadMoneySummary();showToast('❌ Marked as not correct');
}
...
async function submitRepayment(){
  ...
  // Safety guard: paying the exact remaining amount settles the shared debt.
  // Require an explicit second confirmation so an accidental tap/click cannot
  // silently turn an unpaid friend-debt into Paid.
  {
    const {data:currentDebt,error:currentDebtError}=await sb.from('shared_debts_with_computed_status')
      .select('remaining,effective_status,amount').eq('id',debtId).maybeSingle();
    if(currentDebtError){
      err.textContent=currentDebtError.message||'Could not verify the debt before recording repayment';
      return;
    }
    const remaining=Number(currentDebt?.remaining||0);
    if(currentDebt && amount>=remaining-0.005){
      const ok=window.confirm('This repayment will mark the debt as PAID.\n\nConfirm that you have actually paid ₹'+remaining.toFixed(2)+' to your friend.');
      if(!ok){err.textContent='Repayment cancelled — the debt is still unpaid.';return;}
    }
  }
  const userId=await resolveSupabaseUserId();
  if(userId===null||userId===undefined){err.textContent='Could not resolve your account';return;}
...
(function(){
  ...
  window.openEventsMediaModal=async function(){
    if(!(await pvIsAdmin())){showToast('Events & Media is available to administrators only','#ffb300');return;}
  ...
  window.pvRenderHomeSponsoredMedia=async function(){
  ...
  window.pvLoadEventsMedia=async function(){
    ...
    const festivals=(festivalRes.data||[]).filter(f=>!f.festival_date||String(f.festival_date).slice(0,10)>=dayKey);
    const ads=(adRes.data||[]).filter(a=>{
      const start=a.start_date?String(a.start_date).slice(0,10):null;
      const end=a.end_date?String(a.end_date).slice(0,10):null;
  ...
  async function pvRenderAdmin(ads){
    ...
    card.style.display='block';
    const aiCard=document.getElementById('pv-ai-content-card'); if(aiCard) aiCard.style.display='block';
  ...
  window.pvGenerateAdminContent=function(){
    const prompt=(document.getElementById('pv-ai-prompt')?.value||'').trim();
    const out=document.getElementById('pv-ai-output'); if(!out)return;
    if(!prompt){out.innerHTML='<div style="color:var(--red);font-size:12px">Describe the post you want to generate.</div>';return;}
    const q=prompt.toLowerCase();
    let title='ProjectVault Update',body='Stay organized with ProjectVault Expenses Tracker — track every rupee, effortlessly.',kind='announcement';
    if(q.includes('diwali')){title='🪔 Happy Diwali from ProjectVault';body='Wishing you a bright and prosperous Diwali. Keep your finances organized and every rupee on track with ProjectVault Expenses Tracker.';kind='festival';}
    else if(q.includes('birthday')){title='🎂 Birthday Wishes from ProjectVault';body='Warm birthday wishes from ProjectVault! Have a wonderful day, and keep your financial journey organized with Expenses Tracker.';kind='birthday';}
    else if(q.includes('budget')){title='🎯 Stay on Budget with ProjectVault';body='Set your monthly and category budgets, watch your spending, and make smarter decisions before the month gets away from you.';kind='tip';}
    else if(q.includes('debt')||q.includes('repay')){title='🤝 Keep Repayments on Track';body='Record debts, repayments and due dates in ProjectVault so you always know what remains and what has already been settled.';kind='tip';}
    const safeTitle=escHtml(title),safeBody=escHtml(body);
    out.innerHTML=`<div style="border:1px solid var(--border);border-radius:14px;padding:12px;background:var(--surface2)"><div style="font-size:11px;color:var(--accent);font-weight:700">GENERATED DRAFT · ${kind.toUpperCase()}</div><input id="pv-ai-title" class="ti" value="${safeTitle}" style="width:100%;margin-top:8px"><textarea id="pv-ai-body" class="ti" style="width:100%;min-height:90px;margin-top:8px;resize:vertical">${safeBody}</textarea><button class="add-btn" style="width:100%;margin-top:8px" onclick="pvUseGeneratedContent()">Use This Draft</button><div id="pv-ai-use-status" style="font-size:11px;color:var(--muted);margin-top:7px"></div></div>`;
  };
  window.pvUseGeneratedContent=function(){
    const title=document.getElementById('pv-ai-title')?.value||'',body=document.getElementById('pv-ai-body')?.value||'';
    const target=document.getElementById('adm-festival-greeting'); if(target&&body)target.value=body;
    const status=document.getElementById('pv-ai-use-status'); if(status)status.textContent='Draft copied into the admin editor. Review before publishing.';
  };
...
(function(){
  ...
  function mediaUrl(path){if(!path)return '';return SUPABASE_URL+'/storage/v1/object/public/pv-media/'+String(path).split('/').map(encodeURIComponent).join('/');}
  function mediaHtml(m){const t=String(m?.media_type||'').toLowerCase(),u=mediaUrl(m?.storage_path);if(!u)return '';if(t==='video'||t.startsWith('video/'))return '<video class="pv-fd-media cover" autoplay muted playsinline webkit-playsinline preload="auto" controlslist="nodownload noplaybackrate" src="'+esc(u)+'"></video>';return '<img class="pv-fd-media cover" src="'+esc(u)+'" alt="ProjectVault display media">';}
  ...
  function render(item){
    ...
    const v=c.querySelector('video');if(v){v.muted=true;v.setAttribute('muted','');v.setAttribute('autoplay','');v.setAttribute('playsinline','');v.setAttribute('webkit-playsinline','');v.preload='auto';const startVideo=()=>{v.muted=!sound;if(v.readyState<2){try{v.load();}catch(e){}}const p=v.play();if(p&&typeof p.catch==='function')p.catch(err=>console.warn('[display] video autoplay blocked:',err));};v.addEventListener('loadedmetadata',startVideo,{once:true});v.addEventListener('canplay',startVideo,{once:true});try{v.load();}catch(e){}setTimeout(startVideo,150);}
  }
  async function next(){
    if(!started)return;
  ...
  window.pvOpenDisplay=async function(){
    const el=document.getElementById('pv-full-display');if(!el)return;el.style.display='block';el.setAttribute('aria-hidden','false');started=true;index=0;items=[];await build();await next();
  };
  window.pvCloseDisplay=function(){started=false;clearTimeout(timer);clearInterval(progressTimer);const el=document.getElementById('pv-full-display');if(el){el.style.display='none';el.setAttribute('aria-hidden','true');}};
