...
async function searchDebtUsers(val){
  ...
  debtUserSearchTimer=setTimeout(async()=>{
    try{
      ...
      // Preferred RPC for this merged Debt screen. It is expected to search BOTH username
      // and full name while returning only safe profile fields.
      ({data,error}=await sb.rpc('search_users_for_shared_money',{p_query:q}));
      if(error){
        // Backward-compatible fallback for databases that still only have the old username RPC.
        const fallback=await sb.rpc('search_users_by_username',{p_query:q});
      ...
      if(error){
        ...
        list.innerHTML='<div class="ac-item"><span style="color:var(--red)">User search is not configured yet — check the Supabase migration.</span></div>';
        return;
      }
...
async function saveDebt(){
  ...
  if(sharedError){
    ...
    // Do not leave a misleading local-only record when the cross-account source of truth failed.
    const clean=getDebts().filter(x=>x.ts!==ts); saveDebts(clean); renderDebtPage(); renderHome();
    showToast(sharedError.code==='42501'?'⚠️ Connect with this user before sharing money':'⚠️ Shared money sync failed','#ffb300');
    return;
  }
  const d2=getDebts();
