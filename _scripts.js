...
async function confirmIncomingSharedDebt(debtId){
  ...
  if(row){
    ...
    const userId=await resolveSupabaseUserId();
    const debts=getDebts();
    if(!debts.some(d=>String(d.sharedDebtId)===String(row.id))){
      debts.push({name:senderNames?.[row.lender_id]||row.reason||'Money received',amount:Number(row.amount),type:'debt',ts:new Date(row.created_at).getTime(),cleared:false,dueDate:row.due_date||null,repaid:0,sharedDebtId:row.id,sharedUserId:row.lender_id});
      saveDebts(debts);
    }
  }
